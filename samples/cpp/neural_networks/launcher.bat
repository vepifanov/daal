    ...
    if not "%RMODE%"=="build" (
        for %%U in (%%T %%T_dll) do (
            if exist .\%RESULT_DIR%\%%U.exe (
                .\%RESULT_DIR%\%%U.exe %DATA_PATH%\train-images.idx3-ubyte %DATA_PATH%\train-labels.idx1-ubyte %DATA_PATH%\t10k-images.idx3-ubyte %DATA_PATH%\t10k-labels.idx1-ubyte 1>.\%RESULT_DIR%\%%U.res 2>&1
                if "!errorlevel!" == "0" (
                    echo %time% PASSED %%U
                ) else (
