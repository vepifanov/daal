                       ...
                       size_t nPrintedRows = 0, size_t nPrintedCols = 0, size_t interval = 10)
{
    ...
    else
    {
        ...
        if(isLower(layout))
        {
            printLowerArray<double>(block.getBlockPtr(), nPrintedRows, message, interval);
        }
        else if(isUpper(layout))
        {
            printUpperArray<double>(block.getBlockPtr(), nPrintedCols, nPrintedRows, nCols, message, interval);
        }
                        ...
                        const char *title1 = "", const char *title2 = "", const char *message = "", size_t nPrintedRows = 0, size_t interval = 15)
{
    ...
    for (size_t i = 0; i < nRows; i++)
    {
        ...
        {
            std::cout << std::setprecision(0) << std::setw(interval) << dataDouble2[i * nCols2 + j];
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
...
daal::services::SharedPtr<Tensor> readTensorFromCSV(const std::string &datasetFileName, bool allowOneColumn = false)
{
...
daal::services::SharedPtr<Tensor> getNextSubtensor(daal::services::SharedPtr<Tensor> inputTensor, size_t startPos, size_t nElements)
{
    daal::services::Collection<size_t> dims = inputTensor->getDimensions();
    dims[0] = nElements;

    SubtensorDescriptor<float> subtensorBlock;
    inputTensor->getSubtensor(0, 0, startPos, nElements, readOnly, subtensorBlock);

    HomogenTensor<float> *subtensor = new HomogenTensor<float>(dims, Tensor::doAllocate);
    float *subtensorData = subtensor->getArray();
    float *inputData = subtensorBlock.getPtr();

    for(size_t i = 0; i < subtensor->getSize(); i++)
    {
        subtensorData[i] = inputData[i];
    }

    inputTensor->releaseSubtensor(subtensorBlock);
    daal::services::SharedPtr<Tensor> subtensorPtr(subtensor);
    return subtensorPtr;
}
...
daal::services::SharedPtr<HomogenNumericTable<float> > createTableFromTensor(Tensor &tensor, bool *isCopy = NULL)
{
    HomogenTensor<float> *homogenTensor = dynamic_cast<HomogenTensor<float> *>(&tensor);
    if (homogenTensor)
    {
        if (isCopy)
            *isCopy = false;
        return daal::services::SharedPtr<HomogenNumericTable<float> >(
                new HomogenNumericTable<float>(homogenTensor->getArray(), 1, homogenTensor->getSize()));
    }
    else
    {
        if (isCopy)
            *isCopy = true;
        size_t dataSize = tensor.getSize();
        daal::services::SharedPtr<HomogenNumericTable<float> > nt(
                new HomogenNumericTable<float>(1, dataSize, NumericTable::doAllocate));
        SubtensorDescriptor<float> subtensor;
        tensor.getSubtensor(0, 0, 0, tensor.getDimensionSize(0), readOnly, subtensor);
        float *dst = nt->getArray();
        const float *src = subtensor.getPtr();
...
void copyTableToTensor(NumericTable &nt, Tensor &tensor)
{
    HomogenTensor<float> *homogenTensor = dynamic_cast<HomogenTensor<float> *>(&tensor);
    HomogenNumericTable<float> *homogenTable = dynamic_cast<HomogenNumericTable<float> *>(&nt);
    size_t dataSize = tensor.getSize();
    if (homogenTensor && homogenTable)
    {
        float *dst = homogenTensor->getArray();
        const float *src = homogenTable->getArray();
        if (src == dst)
        {
            return;
        }
    ...
    {
        SubtensorDescriptor<float> subtensor;
        tensor.getSubtensor(0, 0, 0, tensor.getDimensionSize(0), writeOnly, subtensor);
        float *dst = subtensor.getPtr();
        BlockDescriptor<float> block;
        nt.getBlockOfRows(0, nt.getNumberOfRows(), readOnly, block);
        const float *src = block.getBlockPtr();
        for (size_t i = 0; i < dataSize; i++)
        {
            dst[i] = src[i];
        }
