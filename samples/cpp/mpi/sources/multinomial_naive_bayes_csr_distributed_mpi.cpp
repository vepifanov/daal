...
void trainModel()
{
    ...
    CSRNumericTable *trainDataTable = createSparseTable<double>(trainDatasetFileNames[rankId]);
    ...
    localAlgorithm.input.set(classifier::training::data,   services::SharedPtr<CSRNumericTable>(trainDataTable));
...
{
    /* Retrieve the input data from a .csv file */
    CSRNumericTable *testDataTable = createSparseTable<double>(testDatasetFileName);
    ...
    algorithm.input.set(classifier::prediction::data,  services::SharedPtr<CSRNumericTable>(testDataTable));
