/* file: kmeans_init_csr_distributed_mpi.cpp */
/*******************************************************************************
...
{
    return NumericTablePtr(createSparseTable<double>(dataFileNames[rankId]));
}
...
static NumericTablePtr allToAll(const NumericTablePtr& value)
{
    ...
    SharedPtr<RowMergedNumericTable> pMerged(new RowMergedNumericTable());
...
NumericTablePtr initStep1(int rankId, const NumericTablePtr& pData)
{
    ...
    kmeans::init::Distributed<step1Local, double, method> local(nClusters, nBlocks*nVectorsInBlock, rankId*nVectorsInBlock);
    local.input.set(kmeans::init::data, pData);
    local.compute();
    return allToAll(local.getPartialResult()->get(kmeans::init::partialClusters));
}
...
template <kmeans::init::Method method>
NumericTablePtr initStep3(kmeans::init::Distributed<step3Master, double, method>& step3, std::vector<NumericTablePtr>& step2Results)
{
...
NumericTablePtr initCentroids<kmeans::init::plusPlusCSR>(int rankId, const NumericTablePtr& pData)
{
    ...
    SharedPtr<RowMergedNumericTable> pCentroids(new RowMergedNumericTable());
    ...
    typedef kmeans::init::Distributed<step3Master, double, method> Step3Master;
    ...
    return daal::data_management::convertToHomogen<double>(*pCentroids); /* can be returned as pCentroids as well */
}
...
NumericTablePtr initCentroids<kmeans::init::parallelPlusCSR>(int rankId, const NumericTablePtr& pData)
{
    ...
    typedef kmeans::init::Distributed<step5Master, double, method> Step5Master;
...
NumericTablePtr computeCentroids(int rankId, const NumericTablePtr& pData, const NumericTablePtr& initialCentroids)
{
    ...
    kmeans::Distributed<step1Local, double, kmeans::lloydCSR> localAlgorithm(nClusters);
    ...
    if(isRoot)
    {
        ...
        for(size_t i = 0; i < nBlocks; i++)
        {
            ...
            SharedPtr<kmeans::PartialResult> dataForStep2FromStep1 = dynamicPointerCast<kmeans::PartialResult, SerializationIface>(ptr);
