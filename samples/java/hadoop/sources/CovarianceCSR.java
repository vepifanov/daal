...
public class CovarianceCSR extends Configured implements Tool {

    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new CovarianceCSR(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = new Job(conf, "CovarianceCSR Job");

        FileInputFormat.setInputPaths(job, new Path(args[0]));
        FileOutputFormat.setOutputPath(job, new Path(args[1]));

        job.setMapperClass(CovarianceCSRStep1Mapper.class);
        job.setReducerClass(CovarianceCSRStep2Reducer.class);
        ...
        job.setJarByClass(CovarianceCSR.class);
