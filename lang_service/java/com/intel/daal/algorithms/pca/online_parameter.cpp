...
(JNIEnv *env, jobject thisObj, jlong parAddr, jlong initAddr, jint method, jint cmode, jint computeStep, jint prec)
{
    ...
    else if(method == SVDDenseValue)
    {
        if(prec == 0) //double
        {
            OnlineParameter<double, svdDense> *parameterAddrCor = (OnlineParameter<double, svdDense> *)parAddr;
            parameterAddrCor->initializationProcedure = *((SharedPtr<PartialResultsInitIface<svdDense> > *)initAddr);
        }
        else if (prec == 1) //float
        {
            OnlineParameter<float, svdDense> *parameterAddrCor = (OnlineParameter<float, svdDense> *)parAddr;
            parameterAddrCor->initializationProcedure = *((SharedPtr<PartialResultsInitIface<svdDense> > *)initAddr);
        }
    }
}
