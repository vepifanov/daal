...
public:
        ...
        for(size_t i = 0; i < nFeatures; i++)
        {
            ...
            switch(column->t)
            {
                ...
                case(KE):
                    ...
                    break;
                case(KB): case(KG): case(KC):
                    feature.setType<char>();
                    break;
                case(KH):
                    feature.setType<short>();
                    break;
                case(KI): case(KM): case(KD): case(KU): case(KV): case(KT):
                    feature.setType<int>();
                    break;
                case(KJ):
                    feature.setType<DAAL_INT64>();
                    break;
                default:
                    _errors->add(services::ErrorKDBTypeUnsupported);
                    break;
            }
        }
    }
    ...
    void createDictionaryFromList(const K& lst, DataSourceDictionary *dict)
    {
        ...
        for(size_t i = 0; i < nFeatures; i++)
        {
            ...
            if (lst->t == 0) curType = kK(lst)[i]->t; else curType = lst->t;
    ...
    void statementResultsNumericTableFromColumnData(const K& columnData, NumericTable *nt, size_t nRows)
    {
        ...
        double *blockPtr = block.getBlockPtr();
        ...
        for (size_t row = 0; row < nRows; row++)
        {
            for(size_t col = 0; col < nFeatures; col++)
            {
                ...
                {
                    case(KB):
                        blockPtr[row * nFeatures + col] = kG(column)[row];
                        break;
                    case(KG):
                        blockPtr[row * nFeatures + col] = kG(column)[row];
                        break;
                    case(KH):
                        blockPtr[row * nFeatures + col] = kH(column)[row];
                        break;
                    case(KI): case(KM): case(KD): case(KU): case(KV): case(KT):
                        blockPtr[row * nFeatures + col] = kI(column)[row];
                        break;
                    case(KJ): case(KP): case(KN):
                        blockPtr[row * nFeatures + col] = kJ(column)[row];
                        break;
                    case(KE):
                        blockPtr[row * nFeatures + col] = kE(column)[row];
                        break;
                    case(KF):
                        blockPtr[row * nFeatures + col] = kF(column)[row];
                        break;
                    default:
                        _errors->add(services::ErrorKDBTypeUnsupported);
                        break;
    ...
    void statementResultsNumericTableFromList(const K& lst, NumericTable *nt, size_t nRows)
    {
        ...
        {
            K data;
            if (lst->t == 0) data = kK(lst)[row]; else data = lst;
            for(size_t col = 0; col < nFeatures; col++)
            {
                switch(data->t)
                {
                    case(KB):
                        blockPtr[row * nFeatures + col] = kG(data)[col];
                        break;
                    case(KG):
                        blockPtr[row * nFeatures + col] = kG(data)[col];
                        break;
                    case(KH):
                        blockPtr[row * nFeatures + col] = kH(data)[col];
                        break;
                    case(KI): case(KM): case(KD): case(KU): case(KV): case(KT):
                        blockPtr[row * nFeatures + col] = kI(data)[col];
                        break;
                    case(KJ): case(KP): case(KN):
                        blockPtr[row * nFeatures + col] = kJ(data)[col];
                        break;
                    case(KE):
                        blockPtr[row * nFeatures + col] = kE(data)[col];
                        break;
                    case(KF):
                        blockPtr[row * nFeatures + col] = kF(data)[col];
                        break;
                    default:
                        _errors->add(services::ErrorKDBTypeUnsupported);
                        break;
