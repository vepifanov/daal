...
class KDBDataSource : public DataSourceTemplate<data_management::HomogenNumericTable<double>, summaryStatisticsType>
{
public:
...
public:
    ...
    void createDictionaryFromContext() DAAL_C11_OVERRIDE
    {
        ...
        if (!result)
        {
            this->_errors->add(services::ErrorKDBNetworkError);
            _kdbClose(handle);
            return;
        }
        ...
        if (result->t == -128)
        {
            this->_errors->add(services::ErrorKDBServerError);
            r0(result);
            _kdbClose(handle);
            return;
        }
        ...
        if (result->t == XT)
        {
            featureManager.createDictionaryFromTable(result->k, _dict);
        }
        else if (result->t == XD)
        {
            featureManager.createDictionaryFromTable(result, _dict);
        }
        else
        {
            featureManager.createDictionaryFromList(kK(result)[0], _dict);
        }
        r0(result);
