...
public:
    ...
    static services::SharedPtr<HomogenNumericTable<DataType> > create(DictionaryIface::FeaturesEqual featuresEqual,
                                                                      DataType *const ptr = 0, size_t nColumns = 0, size_t nRows = 0,
                                                                      services::Status *stat = NULL)
    {
    ...
    static services::SharedPtr<HomogenNumericTable<DataType> > create(DictionaryIface::FeaturesEqual featuresEqual,
                                                                      ...
                                                                      size_t nColumns, size_t nRows, services::Status *stat = NULL)
    {
     ...
     */
    static services::SharedPtr<HomogenNumericTable<DataType> > create(DataType *const ptr, size_t nColumns, size_t nRows, const DataType &constValue,
                                                                      services::Status *stat = NULL)
    {
     ...
     */
    static services::SharedPtr<HomogenNumericTable<DataType> > create(DictionaryIface::FeaturesEqual featuresEqual,
                                                                      size_t nColumns, size_t nRows, AllocationFlag memoryAllocationFlag,
                                                                      services::Status *stat = NULL)
    {
...
protected:
    ...
    HomogenNumericTable(services::Status &st):
        NumericTable(0, 0, DictionaryIface::notEqual, st)
    {}

    HomogenNumericTable(NumericTableDictionaryPtr ddictForHomogenNumericTable, services::Status &st):
        NumericTable(ddictForHomogenNumericTable, st)
    {
        _layout = aos;
    }

    HomogenNumericTable(DictionaryIface::FeaturesEqual featuresEqual, const services::SharedPtr<DataType> &ptr,
    ...
    HomogenNumericTable(DictionaryIface::FeaturesEqual featuresEqual, const services::SharedPtr<DataType> &ptr,
                        size_t nColumns, size_t nRows, const DataType &constValue, services::Status &st):
        NumericTable(nColumns, nRows, featuresEqual, st)
    {
        _layout = aos;
        st |= setArray( ptr, nRows );
    ...
    HomogenNumericTable(DictionaryIface::FeaturesEqual featuresEqual,
                        size_t nColumns, size_t nRows, AllocationFlag memoryAllocationFlag, services::Status &st):
        NumericTable(nColumns, nRows, featuresEqual, st)
    {
        _layout = aos;
...
protected:
    ...
    services::Status getTBlock( size_t idx, size_t nrows, int rwFlag, BlockDescriptor<T> &block )
    {
        ...
        else
        {
            ...
            if( rwFlag & (int)readOnly )
            {
                ...
                {
                    data_feature_utils::getVectorUpCast(data_feature_utils::getIndexNumType<DataType>(), data_feature_utils::getInternalNumType<T>())
    ...
    services::Status releaseTBlock( BlockDescriptor<T> &block )
    {
        if(block.getRWFlag() & (int)writeOnly)
        {
            ...
            else
            {
                for(size_t i = 0; i < nrows; i++)
                {
                    data_feature_utils::getVectorDownCast(data_feature_utils::getIndexNumType<DataType>(), data_feature_utils::getInternalNumType<T>())
    ...
    services::Status getTFeature(size_t feat_idx, size_t idx, size_t nrows, int rwFlag, BlockDescriptor<T> &block)
    {
        ...
        else
        {
            ...
            if( rwFlag & (int)readOnly )
            {
                ...
                data_feature_utils::getVectorStrideUpCast(data_feature_utils::getIndexNumType<DataType>(), data_feature_utils::getInternalNumType<T>())
    ...
    services::Status releaseTFeature( BlockDescriptor<T> &block )
    {
        if (block.getRWFlag() & (int)writeOnly)
        {
            ...
            data_feature_utils::getVectorStrideDownCast(data_feature_utils::getIndexNumType<DataType>(), data_feature_utils::getInternalNumType<T>())
