...
private:
    ...
    void getTFeature( size_t feat_idx, size_t idx, size_t nrows, int rwFlag, BlockDescriptor<T>& block )
    {
        ...
        else
        {
            ...
            data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorUpCast(f.indexType,data_feature_utils::getInternalNumType<T>());
            if (convertFunc)
                convertFunc( nrows, location, block.getBlockPtr() );
            else
                return;
        }
    }
    ...
    void releaseTFeature( size_t feat_idx, size_t idx, size_t nrows, T *buf, ReadWriteMode rwFlag )
    {
        if (rwFlag & (int)writeOnly)
        {
            ...
            if( data_feature_utils::getIndexNumType<T>() != f.indexType )
            {
                ...
                data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
                if (convertFunc)
                    convertFunc( nrows, buf, location );
                else
                    return;
            }
        }
    ...
    void releaseTFeature( BlockDescriptor<T>& block )
    {
        if (block.getRWFlag() & (int)writeOnly)
        {
            ...
            if( data_feature_utils::getIndexNumType<T>() != f.indexType )
            {
                ...
                data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
                if (convertFunc)
                    convertFunc( block.getNumberOfRows(), block.getBlockPtr(), ptr );
                else
                    return;
            }
        }
