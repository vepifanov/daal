...
private:
    ...
    void getTBlock( size_t idx, size_t nrows, int rwFlag, BlockDescriptor<T>& block )
    {
        ...
        for( size_t j = 0 ; j < ncols ; j++ )
        {
            ...
            data_feature_utils::vectorStrideConvertFuncType convertFunc = data_feature_utils::getVectorStrideUpCast(f.indexType,data_feature_utils::getInternalNumType<T>());
            if (convertFunc)
                convertFunc( nrows, location, _structSize, blockPtr + j, sizeof(T)*ncols );
            else
                return;
        }
    }
    ...
    void releaseTBlock( size_t idx, size_t nrows, T *buf, int rwFlag )
    {
        if (rwFlag & (int)writeOnly)
        {
            ...
            for( j = 0 ; j < ncols ; j++ )
            {
                ...
                data_feature_utils::vectorStrideConvertFuncType convertFunc = data_feature_utils::getVectorStrideDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
                if (convertFunc)
                    convertFunc( nrows, buf + j, sizeof(T)*ncols, location, _structSize );
                else
                    return;
            }
        }
    ...
    void releaseTBlock( BlockDescriptor<T>& block )
    {
        if(block.getRWFlag() & (int)writeOnly)
        {
            ...
            for( size_t j = 0 ; j < ncols ; j++ )
            {
                ...
                data_feature_utils::vectorStrideConvertFuncType convertFunc = data_feature_utils::getVectorStrideDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
                if (convertFunc)
                    convertFunc( block.getNumberOfRows(), blockPtr + j, sizeof(T)*ncols, location, _structSize );
                else
                    return;
            }
        }
    ...
    void getTFeature( size_t feat_idx, size_t idx, size_t nrows, int rwFlag, BlockDescriptor<T>& block )
    {
        ...
        data_feature_utils::vectorStrideConvertFuncType convertFunc = data_feature_utils::getVectorStrideUpCast(f.indexType,data_feature_utils::getInternalNumType<T>());
        if (convertFunc)
            convertFunc( nrows, ptr, _structSize, block.getBlockPtr(), sizeof(T) );
        else
            return;
    }
    ...
    void releaseTFeature( size_t feat_idx, size_t idx, size_t nrows, T *buf, int rwFlag )
    {
        if (rwFlag & (int)writeOnly)
        {
            ...
            data_feature_utils::vectorStrideConvertFuncType convertFunc = data_feature_utils::getVectorStrideDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
            if (convertFunc)
                convertFunc( nrows, buf, sizeof(T), location, _structSize );
            else
                return;
        }
    }
    ...
    void releaseTFeature( BlockDescriptor<T>& block )
    {
        if (block.getRWFlag() & (int)writeOnly)
        {
            ...
            data_feature_utils::vectorStrideConvertFuncType convertFunc = data_feature_utils::getVectorStrideDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
            if (convertFunc)
                convertFunc( block.getNumberOfRows(), block.getBlockPtr(), sizeof(T), ptr, _structSize );
            else
                return;
        }
        block.setDetails( 0, 0, 0 );
