...
protected:
    ...
    template<typename StructDataType>
    AOSNumericTable(const services::SharedPtr<StructDataType> &ptr, size_t ncol, size_t nrow, services::Status &st) : NumericTable(ncol, nrow, DictionaryIface::notEqual, st)
    {
        _ptr        = services::reinterpretPointerCast<byte, StructDataType>(ptr);
        _layout     = aos;
        _structSize = sizeof(StructDataType);
