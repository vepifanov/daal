...
public:
    ...
    CSRBlockDescriptor() : _values_ptr(0), _rows_ptr(0), _cols_ptr(0),
        ...
        _ncols(0), _nrows(0), _rowsOffset(0), _rwFlag(0), _nvalues(0) {}
...
protected:
    ...
    void getTBlock( size_t idx, size_t nrows, int rwFlag, BlockDescriptor<T> &block )
    {
        ...
        else
        {
            ...
            data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorUpCast(f.indexType,data_feature_utils::getInternalNumType<T>());
            if (convertFunc)
                convertFunc( sparseBlockSize, location, castingBuffer );
            else
                return;
        }
        ...
        if (!bufRowCursor) return;  /*This check should go after the cycle with buffer filling in*/
        ...
        for(size_t i = 0; i < nrows; i++)
        {
            ...
            for(size_t k = 0; k < sparseRowSize; k++)
            {
                if( indicesCursor[k] - 1 == feat_idx )
                {
                    data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorUpCast(f.indexType,data_feature_utils::getInternalNumType<T>());
                    if (convertFunc)
                        convertFunc( 1, rowCursor + k * f.typeSize, buffer + i );
                    else
                        return;
                }
            }
    ...
    void getSparseTBlock( size_t idx, size_t nrows, int rwFlag, CSRBlockDescriptor<T> &block )
    {
        ...
        else
        {
            ...
            data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorUpCast(f.indexType,data_feature_utils::getInternalNumType<T>());
            if (convertFunc)
                convertFunc( nValues, location, block.getBlockValuesPtr() );
            else
                return;
        }
    ...
    void releaseSparseTBlock(CSRBlockDescriptor<T> &block)
    {
        if (block.getRWFlag() & (int)writeOnly)
        {
            ...
            if (f.indexType != data_feature_utils::getIndexNumType<T>())
            {
                ...
                data_feature_utils::vectorConvertFuncType convertFunc = data_feature_utils::getVectorDownCast(f.indexType,data_feature_utils::getInternalNumType<T>());
                if (convertFunc)
                    convertFunc( nValues, ptr, location );
                else
                    return;
            }
        }
