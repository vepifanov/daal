...
public:
     ...
     */
    Model(const services::SharedPtr<neural_networks::ForwardLayers> &forwardLayersForModel,
    ...
    DAAL_EXPORT Model(const services::SharedPtr<neural_networks::ForwardLayers> &forwardLayersForModel,
                      const services::SharedPtr<services::Collection<layers::NextLayers> > &nextLayersForModel,
    ...
    void allocate(const services::Collection<size_t> &dataSize, const daal::algorithms::Parameter *parameter = NULL)
    {
        ...
        if (_allocatedBatchSize == par->batchSize) { return; }
...
protected:
    ...
    {
        services::SharedPtr<layers::forward::LayerIface> forwardLayer = layerDescriptor.layer->clone();
