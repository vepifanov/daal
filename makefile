...
spat = %.java %.cpp %.h %.txt %.csv %.py %.html %.png %.parquet
...
CORE.incdirs.core   := $(CORE.srcdir)  $(addprefix $(CORE.srcdir)/, $(CORE.ALGORITHMS.FULL)) $(addprefix $(CORE.SERV.srcdir)/, $(CORE.SERVICES)) ## change CORE.ALGORITHMS.FULL --> CORE.ALGORITHMS
...
$(CORE.objs_y): COPT += -D__DAAL_IMPLEMENTATION -D__TBB_NO_IMPLICIT_LINKAGE -DDAAL_NOTHROW_EXCEPTIONS $(if $(CHECK_DLL_SIG),-DDAAL_CHECK_DLL_SIG)
