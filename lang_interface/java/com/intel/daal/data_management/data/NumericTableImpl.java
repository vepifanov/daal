...
abstract public class NumericTableImpl extends SerializableBase {
     ...
     */
    public int getIntValue(long column, long row) {
        checkCObject();
        return cGetIntValue(cObject, column, row);
    }

    private native int cGetIntValue(long cObject, long column, long row);
     ...
     * @return Value from numeric table
     */
    public float getFloatValue(long column, long row) {
        checkCObject();
        return cGetFloatValue(cObject, column, row);
    }

    private native float cGetFloatValue(long cObject, long column, long row);
     ...
     * @return Value from numeric table
     */
    public double getDoubleValue(long column, long row) {
        checkCObject();
        return cGetDoubleValue(cObject, column, row);
    }

    private native double cGetDoubleValue(long cObject, long column, long row);

    @Override
    protected void onPack() {
        nSerializedFeatures = getNumberOfColumns();
        nSerializedVectors = getNumberOfRows();
        serializedFeaturesEqual = dict.getFeaturesEqual();
    }
    ...
    protected long newJavaNumericTable(long nColumns, long nRows, NumericTable.StorageLayout layout, DataDictionary.FeaturesEqual featuresEqual, SerializationTag tag) {
        nJavaFeatures = nColumns;
        nJavaVectors = nRows;
        return cNewJavaNumericTable(nColumns, nRows, layout.ordinal(), featuresEqual.ordinal(), tag.getValue());
    }

    private native long cNewJavaNumericTable(long nColumns, long nRows, int layout, int featuresEqual, int tag);
