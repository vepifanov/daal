...
bool computeMinimumsAndMaximums(low_order_moments::BatchIface *moments, NumericTablePtr &dataTable,
                                NumericTablePtr &minimums, NumericTablePtr &maximums)
{
    ...
    if(!minimums || !maximums)
    {
        ...
        moments->computeNoThrow();
        if(moments->getErrors()->size() != 0) { return false; }
