...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    low_order_moments::BatchIface *moments = parameter->moments.get();

    NumericTablePtr minimums;
    NumericTablePtr maximums;
    if (!internal::computeMinimumsAndMaximums(moments, dataTable, minimums, maximums))
    {
        this->_errors->add(daal::services::ErrorMinAndMaxComputing);
        return;
    }
