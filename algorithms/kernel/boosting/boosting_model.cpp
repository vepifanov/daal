 ...
 */
SharedPtr<weak_learner::Model> Model::getWeakLearnerModel(size_t idx) const
{
    if(idx < _models->size())
    {
        return staticPointerCast<weak_learner::Model, SerializationIface>((*_models)[idx]);
    }
    return SharedPtr<weak_learner::Model>();
}
 ...
 */
void Model::addWeakLearnerModel(SharedPtr<weak_learner::Model> model)
{
    (*_models) << model;
}
