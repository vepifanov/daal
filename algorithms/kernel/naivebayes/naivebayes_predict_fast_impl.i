...
{
    static services::Status getPredictionData(
        const algorithmFPType *aux_table, NumericTable *ntData,
...
services::Status NaiveBayesPredictKernel<algorithmFPType, method, cpu>::compute(const NumericTable *a, const Model *m,
                                                                    NumericTable *r, const Parameter *parameter)
{
    ...
    mkl_buff.reduce( [ = ](algorithmFPType * v)-> void { if(v) _FREE_<algorithmFPType, cpu>( v ); } );

    return safeStat.detach();
}

template<typename algorithmFPType, CpuType cpu>
services::Status methodSpecific<defaultDense, algorithmFPType, cpu>::getPredictionData( const algorithmFPType *aux_table, NumericTable *ntData, size_t n0, size_t n,
                                                                            size_t p, size_t c, int *classes, algorithmFPType *buff )
{
    ...
    {
        int max_c = 0;
        algorithmFPType max_c_val = -(data_feature_utils::internal::MaxVal<algorithmFPType>::get());
...
template<typename algorithmFPType, CpuType cpu>
services::Status methodSpecific<fastCSR, algorithmFPType, cpu>::getPredictionData( const algorithmFPType *aux_table, NumericTable *ntData, size_t n0, size_t n,
                                                                       size_t p, size_t c, int *classes, algorithmFPType *buff )
{
