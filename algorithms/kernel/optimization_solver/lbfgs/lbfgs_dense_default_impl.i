    ...
    OptionalArgument *optionalArgumentResult, Parameter *parameter)
{
    ...
    NumericTablePtr argumentTable(new HomogenNumericTableCPU<algorithmFPType, cpu>(task.argument, 1, task.argumentSize));
            ...
            algorithmFPType *argument)
{
    ...
    ntGradient->getBlockOfRows(0, this->argumentSize, readOnly, gradientBlock);
            ...
            services::SharedPtr<daal::internal::HomogenNumericTableCPU<int, cpu> > &ntBatchIndices)
{
    ...
    {
        int errCode = _rng.uniformWithoutReplacement(batchSize, *batchIndices, _baseGen, 0, (int)nTerms);
...
mtArgument(argumentTable), argument(nullptr), argumentSize(argumentTable->getNumberOfRows())
{
...
{
    /* Initialize work value with a start value provided by user */
    mtArgument.getBlockOfRows(0, argumentSize, &argument);

    BlockMicroTable<algorithmFPType, readOnly, cpu> mtStartValue(startValueTable);
    algorithmFPType *startValue;
    mtStartValue.getBlockOfRows(0, argumentSize, &startValue);
...
LBFGSTask<algorithmFPType, cpu>::LBFGSTask(NumericTable *inputArgument, NumericTable* correctionPairsInput, NumericTable* averageArgLIterInput,
    ...
    _baseGen(parameter->seed), _rng(), _rngStateChanged(false), _rngStateRequired(false)
{
    ...
    argumentLCurTable = SharedPtr<HomogenNumericTableCPU<algorithmFPType, cpu> >(
        new HomogenNumericTableCPU<algorithmFPType, cpu>(argumentLCur, 1, this->argumentSize));
                                                  ...
                                                  size_t nIterations, size_t epoch, size_t corrIndex)
{
    ...
    auto stateSize = _baseGen.getStateSize();
    if(!stateSize)
        return;
    ...
    int errCode = _baseGen.saveState(pState->get());
    if(errCode) { this->_errors->add(ErrorIncorrectErrorcodeFromGenerator); }
}
...
void LBFGSTask<algorithmFPType, cpu>::initRngState(OptionalArgument *optionalArgumentInput, const Parameter* parameter)
{
    ...
    if(!pState || !pState->size())
        return;
    int errCode = _baseGen.loadState(pState->get());
    if(errCode) { this->_errors->add(ErrorIncorrectErrorcodeFromGenerator); }
}
