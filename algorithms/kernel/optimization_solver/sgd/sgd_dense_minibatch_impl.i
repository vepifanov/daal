    ...
    NumericTable *batchIndices, OptionalArgument *optionalArgument, OptionalArgument *optionalResult, engines::BatchBase &engine)
{
    ...
    DAAL_CHECK_STATUS(s, task.init(inputArgument, learningRateSequence, conservativeSequence,
        nIterations, batchIndices, optionalArgument));
    ...
    for(size_t epoch = task.startIteration; s.ok() && (epoch < (task.startIteration + nIter)); epoch++)
    {
        ...
        if(epoch % L == 0)
        {
            ...
            daal_memcpy_s(task.prevWorkValue.get(), argumentSize * sizeof(algorithmFPType), workValue, argumentSize * sizeof(algorithmFPType));
        }
...
Status SGDMiniBatchTask<algorithmFPType, cpu>::setStartValue(NumericTable *startValueTable)
{
    ...
    daal_memcpy_s(mtWorkValue.get(), argumentSize * sizeof(algorithmFPType), mtStartValue.get(), argumentSize * sizeof(algorithmFPType));
    return Status();
}
...
template<typename algorithmFPType, CpuType cpu>
Status SGDMiniBatchTask<algorithmFPType, cpu>::init(NumericTable *startValueTable,
    NumericTable *learningRateSequenceTable,
    NumericTable *conservativeSequenceTable,
    NumericTable *nIterationsTable,
    NumericTable *batchIndicesTable,
    OptionalArgument *optionalArgument)
{
    ...
    if(pastWorkValueInput)
    {
        ...
        daal_memcpy_s(prevWorkValue.get(), argumentSize * sizeof(algorithmFPType), pastWorkValueInputArray, argumentSize * sizeof(algorithmFPType));
    }
    return Status();
}
