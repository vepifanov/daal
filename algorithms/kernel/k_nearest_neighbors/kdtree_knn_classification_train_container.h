...
void BatchContainer<algorithmFpType, method, cpu>::compute()
{
    ...
    const services::SharedPtr<NumericTable> x = input->get(classifier::training::data);
    const services::SharedPtr<NumericTable> y = input->get(classifier::training::labels);

    const services::SharedPtr<kdtree_knn_classification::Model> r = result->get(classifier::training::model);
