...
services::Status EMKernel<algorithmFPType, method, cpu>::compute(
    ...
    const Parameter &par)
{
    EMKernelTask<algorithmFPType, method, cpu> kernelTask(dataTable,
            initialWeights, initialMeans, initialCovariances,
            resultWeights, resultMeans, resultCovariances,
            resultNIterations,
            resultGoalFunction,
            par);
            ...
            for(size_t k = 0; k < nComponents; k++)
            {
                if(e->mergedWSums[k] > MinVal<algorithmFPType>::get())
                {
                    stepM_mergePartialSums(
                        covs->getSigma(k),     &e->mergedPartialCP[k * covs->getOneCovSize()],
...
template<>                         inline float           exp_threshold<float> (void) { return float (-75.0);  }
template<>                         inline double          exp_threshold<double>(void) { return double(-650.0); }
 ...
 */
template<typename algorithmFPType, Method method, CpuType cpu>
void EMKernelTask<algorithmFPType, method, cpu>::stepE(const size_t nVectorsInCurrentBlock, Task<algorithmFPType, cpu> &t, em_gmm::CovarianceStorageId covType)
{
    ...
    else
    {
        ...
        for(size_t k = 0; k < nComponents; k++)
        {
            for(size_t j = 0; j < nFeatures; j++)
            {
                ...
                for(size_t i = 0; i < nVectorsInCurrentBlock; i++)
                {
                    t.x_mu[j * nVectorsInCurrentBlock + i] = t.trans_data[j * nVectorsInCurrentBlock + i] - t.means[k * nFeatures + j]; // transposed x_mu
                }
            }
...
template<typename algorithmFPType, Method method, CpuType cpu>
algorithmFPType EMKernelTask<algorithmFPType, method, cpu>::computePartialLogLikelyhood(const size_t nVectorsInCurrentBlock, Task<algorithmFPType, cpu> &t)
{
 ...
 */
template<typename algorithmFPType, Method method, CpuType cpu>
Status EMKernelTask<algorithmFPType, method, cpu>::stepM_partial(const size_t nVectorsInCurrentBlock, Task<algorithmFPType, cpu> &t, em_gmm::CovarianceStorageId covType)
{
    const size_t nFeatures = t.nFeatures;
    ...
    for(size_t k = 0; k < t.nComponents; k++)
    {
        ...
        int errcode = t.covs->computeThreadPartialResults(dataBlock, &t.w[k * nVectorsInCurrentBlock],
 ...
 */
template<typename algorithmFPType, Method method, CpuType cpu>
EMKernelTask<algorithmFPType, method, cpu>::EMKernelTask(NumericTable &dataTable,
        NumericTable &initialWeights, NumericTable &initialMeans, NumericTable **initialCovariances,
        NumericTable &resultWeights, NumericTable &resultMeans, NumericTable **resultCovariances,
        NumericTable &resultNIterations,
        NumericTable &resultGoalFunction,
        const Parameter &par) :
    dataTable(dataTable),
    initialWeights(initialWeights), initialMeans(initialMeans), initialCovariances(initialCovariances),
    resultWeights(resultWeights), resultMeans(resultMeans), resultCovariances(resultCovariances),
 ...
 */
template<typename algorithmFPType, Method method, CpuType cpu>
void EMKernelTask<algorithmFPType, method, cpu>::stepM_mergePartialSums(
    ...
    daal::threader_for( nComponents, nComponents, [ =, &sigma_buff, &safeStat ](size_t iComp)
    {
        ...
        if (info != 0)
        {
            ...
            if(info != 0)
            {
                ...
                else { e = Error::create(ErrorEMIllConditionedCovarianceMatrix); }
                e->addIntDetail(Component, iComp);
        ...
        if (info != 0)
        {
            ErrorPtr e;
            if(info < 0) { e = Error::create(ErrorIncorrectInternalFunctionParameter); }
            else { e = Error::create(ErrorEMMatrixInverse); }
            e->addIntDetail(Component, iComp);
            e->addIntDetail(Minor, info);
            safeStat.add(e);
            return;
