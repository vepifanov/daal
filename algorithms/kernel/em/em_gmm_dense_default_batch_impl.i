    ...
    const Parameter *par)
{
    ...
    TArray<WriteRows<algorithmFPType, cpu, NumericTable>, cpu> covsPtr(nComponents);
    ...
    TArray<algorithmFPType, cpu> logLikelyhoodLocalArrayPtr(nBlocks);
...
void stepE(const size_t nVectorsInCurrentBlock, Task<algorithmFPType, cpu> &t, const em_gmm::CovarianceStorageId covType)
{
    ...
    if(covType == diagonal)
    {
        for(size_t k = 0; k < nComponents; k++)
        {
            ...
            algorithmFPType *invSigma = (t.covs->getSigma())[k];
