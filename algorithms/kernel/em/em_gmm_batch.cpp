...
Parameter::Parameter(const size_t _nComponents,
                     const SharedPtr<covariance::BatchIface> &_covariance,
...
void Input::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    for(size_t i = 0; i < nComponents; i++)
    {
        ...
        {
            if (!checkNumericTable(nt.get(), this->_errors.get(), inputCovariancesStr(), unexpectedLayoutCovariance, 0, nFeatures, 1)) { return; }
        }
    }
}
...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    for(size_t i = 0; i < nComponents; i++)
    {
        ...
        {
            if (!checkNumericTable(nt.get(), this->_errors.get(), covarianceStr(), unexpectedLayoutCovariance, 0, nFeatures, 1)) { return; }
        }
    }
}
