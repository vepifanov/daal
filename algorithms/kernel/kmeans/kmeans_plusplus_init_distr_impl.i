...
public:
    ...
    bool run(size_t iFirstOfNewCandidates,
...
protected:
        ...
        algorithmFPType& overallError, size_t nBlocks)
    {
        ...
        {
            aMinDistAcc[iBlock] = _data.updateMinDistInBlock(nullptr,
...
void KMeansinitStep2LocalKernel<method, algorithmFPType, cpu>::compute(const DistributedStep2LocalPlusPlusParameter* par,
    ...
    NumericTable* pRes, NumericTable* pOutputForStep5)
{
    ...
    {
        bRes = TaskStep2Local<algorithmFPType, cpu, DataHelperDense<algorithmFPType, cpu> >(pData).run(
            iFirstOfNewCandidates, pNewCenters, pMinDist, overallError);
    }
    else if(method == plusPlusCSR || (bInitial && (method == parallelPlusCSR)))
    {
        bRes = TaskStep2Local<algorithmFPType, cpu, DataHelperCSR<algorithmFPType, cpu> >(pData).run(
            iFirstOfNewCandidates, pNewCenters, pMinDist, overallError);
    }
    else if(method == parallelPlusDense)
    {
        bRes = TaskStep2LocalParallelPlus<algorithmFPType, cpu, DataHelperDense<algorithmFPType, cpu> >(pData, aLocalData).run(
            iFirstOfNewCandidates, pNewCenters, pMinDist, overallError);
    }
    else if(method == parallelPlusCSR)
    {
        bRes = TaskStep2LocalParallelPlus<algorithmFPType, cpu, DataHelperCSR<algorithmFPType, cpu> >(pData, aLocalData).run(
            iFirstOfNewCandidates, pNewCenters, pMinDist, overallError);
    }
    if(!bRes)
    {
...
template <typename algorithmFPType, CpuType cpu>
NumericTablePtr createTableSingleRow(size_t nCols, const algorithmFPType* aVal)
{
...
template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansinitStep3MasterKernel<method, algorithmFPType, cpu>::init(size_t seed, MemoryBlock* pRngState)
{
    _rngState = pRngState;
    if(!_brng)
    {
    ...
    if(_brng->loadState(pRngState->get()))
        this->_errors->add(ErrorIncorrectErrorcodeFromGenerator);
}

template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansinitStep3MasterKernel<method, algorithmFPType, cpu>::saveRngState()
{
    if(!_rngState)
        return;
...
template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansinitStep3MasterKernel<method, algorithmFPType, cpu>::compute(const Parameter* par,
    const KeyValueDataCollection* pInputColl, MemoryBlock* pRngState, KeyValueDataCollection* pOutputColl)
{
    pOutputColl->clear();
    init(par->seed, pRngState);
    ...
    if(outputSize == 1)
    {
        (*pOutputColl)[pOutput->key] = createTableSingleRow<algorithmFPType, cpu>(1, &(pOutput->value));
        return;
    }
    for(size_t i = 0; i < outputSize; )
    {
        ...
        (*pOutputColl)[key] = createTableSingleRow<algorithmFPType, cpu>(n, pTmp);
        i += n;
    }
}
...
template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansinitStep4LocalKernel<method, algorithmFPType, cpu>::compute(const NumericTable* pData, const NumericTable* pInput,
...
template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansinitStep5MasterKernel<method, algorithmFPType, cpu>::compute(const data_management::DataCollection* pCandidates,
...
template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansinitStep5MasterKernel<method, algorithmFPType, cpu>::finalizeCompute(const Parameter *par,
