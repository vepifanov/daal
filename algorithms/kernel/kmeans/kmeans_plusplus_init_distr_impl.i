    ...
    NumericTable* pRes, NumericTable* pOutputForStep5)
{
    ...
    if(iFirstOfNewCandidates == 0)
    {
        ...
        daal::services::internal::service_memset<algorithmFPType, cpu>(minDistBD.get(),
            daal::data_feature_utils::internal::MaxVal<algorithmFPType>::get(), nRows);
...
Status KMeansinitStep3MasterKernel<method, algorithmFPType, cpu>::init(const Parameter* par, MemoryBlock* pRngState, engines::BatchBase &engine)
{
    ...
    if(_isFirstIteration) // first iteration?
    {
        if(!_rngState->get())
        {
            ...
            Status s;
            DAAL_CHECK_STATUS(s, (UniformKernelDefault<algorithmFPType, cpu>::compute(algorithmFPType(0.), algorithmFPType(1.), *engineImpl, 1, &dummy)));
...
services::Status KMeansinitStep3MasterKernel<method, algorithmFPType, cpu>::compute(const Parameter* par,
    const KeyValueDataCollection* pInputColl, MemoryBlock* pRngState, KeyValueDataCollection* pOutputColl, engines::BatchBase &engine)
{
    pOutputColl->clear();
    ...
    DAAL_CHECK_STATUS(s, (UniformKernelDefault<algorithmFPType, cpu>::compute(algorithmFPType(0.), overallError, engine, outputSize, aRngValues.get())));
...
services::Status KMeansinitStep5MasterKernel<method, algorithmFPType, cpu>::finalizeCompute(const Parameter *par,
    const NumericTable* ntCand, const NumericTable* ntWeights, const MemoryBlock* pRngState, NumericTable* pCentroids, engines::BatchBase &engine)
{
    ...
    TaskPlusPlusBatch<algorithmFPType, cpu, DataHelperDense<algorithmFPType, cpu> > task(const_cast<NumericTable*>(ntCand),
        weightsBD.get(), pCentroids, par->nClusters, engine);
    return task.run();
}
