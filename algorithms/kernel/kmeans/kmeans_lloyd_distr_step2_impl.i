...
void KMeansDistributedStep2Kernel<method, algorithmFPType, cpu>::compute( size_t na, const NumericTable *const *a,
                                                                          size_t nr, const NumericTable *const *r, const Parameter *par)
{
    ...
    {
        int    *inClusterS0;
        algorithmFPType *inClusterS1;
        algorithmFPType *inTargetFunc;

        BlockMicroTable<int,    readOnly, cpu> mtInClusterS0   ( a[i * 3 + 0] );
        BlockMicroTable<algorithmFPType, readOnly, cpu> mtInClusterS1   ( a[i * 3 + 1] );
        BlockMicroTable<algorithmFPType, readOnly, cpu> mtInTargetFunc  ( a[i * 3 + 2] );
...
template <Method method, typename algorithmFPType, CpuType cpu>
void KMeansDistributedStep2Kernel<method, algorithmFPType, cpu>::finalizeCompute( size_t na, const NumericTable *const *a,
                                                                                  size_t nr, const NumericTable *const *r, const Parameter *par)
{
    BlockMicroTable<int,    readOnly, cpu> mtClusterS0   ( a[0] );
    BlockMicroTable<algorithmFPType, readOnly, cpu> mtClusterS1   ( a[1] );
    BlockMicroTable<algorithmFPType, readOnly, cpu> mtInTargetFunc( a[2] );

    BlockMicroTable<algorithmFPType, writeOnly, cpu> mtClusters ( r[0] );
    BlockMicroTable<algorithmFPType, writeOnly, cpu> mtTargetFunct ( r[1] );
    ...
    int    *clusterS0;
    algorithmFPType *clusterS1;
    algorithmFPType *inTarget;

    algorithmFPType *clusters;
    algorithmFPType *outTarget;
