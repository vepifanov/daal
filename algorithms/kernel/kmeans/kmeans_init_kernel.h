...
{
public:
    void compute(const NumericTable* pData, const Parameter *par,
...
{
public:
    void finalizeCompute(size_t na, const NumericTable *const *a, NumericTable* ntClusters, const Parameter *par);
};
...
{
public:
    void compute(const DistributedStep2LocalPlusPlusParameter* par,
...
public:
    ...
    void compute(const Parameter* par, const KeyValueDataCollection* pInputColl,
        MemoryBlock* pRngState,
        KeyValueDataCollection* pOutputColl);

protected:
    void init(size_t seed, data_management::MemoryBlock* pRngState);
    void saveRngState();
...
{
public:
    void compute(const NumericTable* pData, const NumericTable* pInput, NumericTable** aLocalData, NumericTable* pOutput);
};
...
{
public:
    void compute(const data_management::DataCollection* pCandidates,
        const data_management::DataCollection* pRating,
        NumericTable* pResCand, NumericTable* pResRating);
    void finalizeCompute(const Parameter *par, const NumericTable* ntCand, const NumericTable* ntWeights,
