...
public:
    ...
    void callGemm(const algorithmFPType* pCenters, size_t nRows, size_t nCenters, algorithmFPType* gemmResult)
    {
        ...
        Blas<algorithmFPType, cpu>::xxgemm(&transa, &transb, &_m, &_n, &_k, &alpha, const_cast<algorithmFPType*>(pCenters),
            &lda, const_cast<algorithmFPType*>(_ntDataBD.get()), &ldy, &beta, gemmResult, &ldaty);
    }
...
public:
    ...
    void callGemm(const algorithmFPType* pCenters, size_t nRows, size_t nCenters, algorithmFPType* gemmResult)
    {
        ...
        SpBlas<algorithmFPType, cpu>::xxcsrmm(&transa, &_n, &_c, &_p, &alpha, matdescra,
            pData, (DAAL_INT *)colIdx, (DAAL_INT *)rowIdx,
...
public:
    ...
    algorithmFPType updateMinDistInBlock(const algorithmFPType* aWeights, size_t iStartRow, size_t nRowsToProcess,
...
{
public:
    typedef RNGs<algorithmFPType, cpu> TRng;
    TaskPlusPlusBatchBase(NumericTable *ntData, NumericTable *ntClusters, size_t numClusters, BaseRNGs<cpu>& brng) :
        _data(ntData), _ntClusters(ntClusters),
        _nClusters(numClusters), _brng(brng),
        _aMinDist(ntData->getNumberOfRows()),
        _overallError(data_management::data_feature_utils::getMaxVal<algorithmFPType>()),
...
protected:
    ...
    void updateMinDist(const algorithmFPType* aWeights);
    ...
    void generateProbabilities(size_t iStart, size_t nTotal)
    {
        _rng.uniform(nTotal, _aProbability.get() + iStart, _brng, algorithmFPType(0.), algorithmFPType(1.));
    }
...
public:
    ...
    TaskPlusPlusBatch(NumericTable *ntData, const algorithmFPType* aWeight,
        NumericTable *ntClusters, size_t numClusters, BaseRNGs<cpu>& brng) :
        super(ntData, ntClusters, numClusters, brng), _aWeight(aWeight)
    {
...
public:
    ...
    bool updateMinDist(size_t iFirstOfNewCandidates, size_t nNewCandidates);

protected:
    void processBlock(size_t iBlock, TlsPPData_t* tlsLocal, size_t iFirstOfNewCandidates, size_t nNewCandidates);
...
public:
    typedef TaskPlusPlusBatchBase<algorithmFPType, cpu, DataHelper> super;
    TaskParallelPlusBatch(NumericTable *ntData, NumericTable *ntClusters, const Parameter *par, BaseRNGs<cpu>& brng) :
        super(ntData, ntClusters, par->nClusters, brng),
...
private:
    ...
    bool getCandidates(HomogenNumericTableCPUPtr& pCandidates);
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
bool TaskPlusPlusBatch<algorithmFPType, cpu, DataHelper>::run()
{
    WriteOnlyRows<algorithmFPType, cpu> clustersBD(this->_ntClusters, 0, this->_nClusters);
    algorithmFPType *clusters = clustersBD.get();
    if(!clusters || !this->_aMinDist.get() || !this->_aMinDistAcc.get() || !this->_lastAddedCenter.get() || !this->_aProbability.get())
        return false;
    daal::services::internal::service_memset<algorithmFPType, cpu>(this->_aMinDist.get(),
        data_feature_utils::getMaxVal<algorithmFPType>(), this->_data.nRows);
...
size_t TaskPlusPlusBatch<algorithmFPType, cpu, DataHelper>::samplePoint(size_t iCluster)
{
    if(this->overallError() > 0)
    {
        ...
        do
        {
            ...
            this->_rng.uniform(1, &probability, this->_brng, algorithmFPType(0.), algorithmFPType(1.));
        }
        while(true);
    }
    return 0;
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
void TaskPlusPlusBatchBase<algorithmFPType, cpu, DataHelper>::updateMinDist(const algorithmFPType* aWeights)
{
    daal::threader_for(_nBlocks, _nBlocks, [=](size_t iBlock)
    {
        _aMinDistAcc.get()[iBlock] = _data.updateMinDistInBlock(aWeights,
...
template <typename algorithmFPType, CpuType cpu>
void KMeansinitKernel<plusPlusDense, algorithmFPType, cpu>::compute(size_t na, const NumericTable *const *a,
    size_t nr, const NumericTable *const *r, const Parameter *par)
{
    BaseRNGs<cpu> brng(par->seed);
...
template <typename algorithmFPType, CpuType cpu>
void KMeansinitKernel<plusPlusCSR, algorithmFPType, cpu>::compute(size_t na, const NumericTable *const *a,
    size_t nr, const NumericTable *const *r, const Parameter *par)
{
    BaseRNGs<cpu> brng(par->seed);
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
void TaskParallelPlusUpdateDist<algorithmFPType, cpu, DataHelper>::processBlock(size_t iBlock, TlsPPData_t* tlsLocal,
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
bool TaskParallelPlusUpdateDist<algorithmFPType, cpu, DataHelper>::updateMinDist(size_t iFirstOfNewCandidates, size_t nNewCandidates)
{
    ...
    daal::threader_for(this->_nBlocks, this->_nBlocks, [=, &tlsData, &bMemoryAllocationFailed](size_t iBlock)
    {
        ...
        processBlock(iBlock, tlsLocal, iFirstOfNewCandidates, nNewCandidates);
    });
    ...
    return !bMemoryAllocationFailed;
}

template <typename algorithmFPType, CpuType cpu, typename DataHelper>
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::updateMinDist(size_t iFirstOfNewCandidates, size_t nNewCandidates)
{
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::getCandidates(HomogenNumericTableCPUPtr& pCandidates)
{
    ...
    _aNearestCandidateIdx.reset(this->_data.nRows);
    if(!_aCandidateIdx.get() || !_aCandidateRating.get() || !_aNearestCandidateIdx.get() || !this->_aProbability.get())
        return false;
    ...
    pCandidates = HomogenNumericTableCPUPtr(new HomogenNumericTableCPU<algorithmFPType, cpu>(this->_data.dim, maxNumberOfCandidates));
    {
        ...
        for(size_t iRound = 0; !bDone; ++iRound)
        {
            ...
            if(nNewCandidates)
            {
                ...
                if(!this->updateMinDist(iFirstNewCandidate, nNewCandidates))
                    return false;
            }
        }
    }
    if(_nCandidates < maxNumberOfCandidates)
        pCandidates->setNumberOfRows(_nCandidates);
    return true;
}

template <typename algorithmFPType, CpuType cpu, typename DataHelper>
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::run()
{
    if(!this->_aMinDist.get() || !this->_aMinDistAcc.get() || !this->_lastAddedCenter.get())
        return false;
    if(!_lastAddedCenterNorm2.get())
        return false;
    daal::services::internal::service_memset<algorithmFPType, cpu>(this->_aMinDist.get(),
        data_management::data_feature_utils::getMaxVal<algorithmFPType>(), this->_data.nRows);
    HomogenNumericTableCPUPtr pCandidates;
    if(!getCandidates(pCandidates))
        return false;
    ...
    TaskPlusPlusBatch<algorithmFPType, cpu, DataHelperDense<algorithmFPType, cpu> > task(pCandidates.get(), aWeight.get(), this->_ntClusters, this->_nClusters, this->_brng);
    return task.run();
}
...
template <typename algorithmFPType, CpuType cpu>
void KMeansinitKernel<parallelPlusDense, algorithmFPType, cpu>::compute(size_t na, const NumericTable *const *a,
    size_t nr, const NumericTable *const *r, const Parameter *par)
{
    BaseRNGs<cpu> brng(par->seed);
...
template <typename algorithmFPType, CpuType cpu>
void KMeansinitKernel<parallelPlusCSR, algorithmFPType, cpu>::compute(size_t na, const NumericTable *const *a,
    size_t nr, const NumericTable *const *r, const Parameter *par)
{
    BaseRNGs<cpu> brng(par->seed);
