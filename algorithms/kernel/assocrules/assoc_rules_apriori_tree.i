...
struct hash_tree
{
    ...
    hash_tree(int h, ItemSetList<cpu> &L_cur) :
    ...
    bool is_child(size_t node_idx, size_t hash_res) const
    {
        return ((1 << hash_res) & root[node_idx].mask);
    }
    ...
    assocrules_itemset<cpu> *hash_subset(size_t iset_size, const size_t *subset, int *levelMiss)
    {
        *levelMiss = 0;
        ...
        for (auto *current = leaves[offset].iset_list.start;
             current != NULL; current = current->next)
        {
            bool found = true;
            size_t *curItems = current->itemSet->items;
            if (!assocrules_memcmp<cpu>(subset, current->itemSet->items, iset_size * sizeof(size_t)))
            {
