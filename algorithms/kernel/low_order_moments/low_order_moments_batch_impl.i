...
void LowOrderMomentsBatchKernel<algorithmFPType, method, cpu>::compute( NumericTable *dataTable,
                                                                        Result *result,
                                                                        const Parameter *parameter )
{
    if( method == defaultDense)
    {
        switch(parameter->estimatesToCompute)
        {
            ...
            {
                estimates_batch_minmax::compute_estimates<algorithmFPType,cpu>(dataTable, result, this->_errors);
                break;
            }
            case estimatesMeanVariance:
            {
                estimates_batch_meanvariance::compute_estimates<algorithmFPType,cpu>(dataTable, result, this->_errors);
                break;
            }
            default /* estimatesAll */:
            {
                estimates_batch_all::compute_estimates<algorithmFPType,cpu>(dataTable, result, this->_errors);
                break;
            }
        }
    ...
    else
    {
        ...
        {
            retrievePrecomputedStatsIfPossible<algorithmFPType, cpu>( task.nFeatures,
                                                                      task.nVectors,
                                                                      dataTable,
        ...
        computeSum_Mean_SecondOrderRawMoment_Variance_Variation<algorithmFPType, method, cpu>( task.nFeatures,
                                                                                               task.nVectors,
                                                                                               task.dataBlock,
                                                                                               task.resultArray[(int)sum],
                                                                                               task.resultArray[(int)mean],
        ...
        computeSumOfSquaredDiffsFromMean<algorithmFPType, cpu>( task.nFeatures,
                                                                ...
                                                                isOnline );
    } /* if( method == defaultDense && do_daal_optimization) */
}
