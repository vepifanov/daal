...
#endif /* #ifdef _THREAD_REDUCTION_ */
...
#if (defined _THREAD_FINAL_) && (defined _STDEV_ENABLE_  || defined _VART_ENABLE_)
    daal::threader_for( numFeatureBlocks, numFeatureBlocks, [ & ](int iFeatureBlock)
    {
        size_t _jstart = iFeatureBlock * numFeaturesInBlock;
        size_t _jend   = _jstart + ((iFeatureBlock < (numFeatureBlocks-1))?numFeaturesInBlock:numFeaturesInLastBlock);
        ...
        for(int j = _jstart; j < _jend; j++)
        {
            _stdev[j] = daal::internal::Math<algorithmFPType, cpu>::sSqrt(_varc[j]);
#ifdef _VART_ENABLE_
            _vart[j]  = _stdev[j] / _mean[j];
#endif
        }
    });
#elif  (defined _STDEV_ENABLE_  || defined _VART_ENABLE_)
    PRAGMA_IVDEP
    PRAGMA_VECTOR_ALWAYS
    PRAGMA_SIMD_ASSERT
    for(int j = 0; j < _cd.nFeatures; j++)
...
#endif /* #if (defined _THREAD_FINAL_) && (defined _STDEV_ENABLE_  || defined _VART_ENABLE_) */
