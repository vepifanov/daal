            ...
            const Parameter *parameter, bool isOnline)
{
    if(method == defaultDense)
    {
        ...
        switch(parameter->estimatesToCompute)
        {
            ...
            {
                estimates_online_minmax::compute_estimates<algorithmFPType, method, cpu>( dataTable, partialResult, isOnline, this->_errors);
                break;
            }
            case estimatesMeanVariance:
            {
                estimates_online_meanvariance::compute_estimates<algorithmFPType, method, cpu>( dataTable, partialResult, isOnline, this->_errors);
                break;
            }
            default /* estimatesAll */:
            {
                estimates_online_all::compute_estimates<algorithmFPType, method, cpu>( dataTable, partialResult, isOnline, this->_errors);
                break;
            }
        }
