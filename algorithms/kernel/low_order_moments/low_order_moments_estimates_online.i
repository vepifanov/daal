...
{
    common_moments_data_t( NumericTable *dataTable,
                               PartialResult *partialResult,
                               bool isOnline,
                               services::SharedPtr<services::KernelErrorCollection> &_errors) : dataTable(dataTable)
    {
        malloc_errors = 0;
        ...
        if (!mean || !variance ) { _errors->add(services::ErrorMemoryAllocationFailed); return; }
#endif
...
struct tls_moments_data_t
{
    ...
    tls_moments_data_t(size_t nFeatures, algorithmFPType* firstRow)
    {
        malloc_errors = 0;
        nvectors = 0;
...
void compute_estimates( NumericTable *dataTable,
                        PartialResult *partialResult,
                        bool isOnline,
                        services::SharedPtr<services::KernelErrorCollection> &_errors )
{
    common_moments_data_t<algorithmFPType,cpu> _cd(dataTable,partialResult,isOnline,_errors);
    ...
    size_t nObs = (size_t)(_cd.resultArray[(int)nObservations][0]);
    ...
    if (!isOnline)
    {
        size_t rowSize = _cd.nFeatures * sizeof(algorithmFPType);

#if (defined _MIN_ENABLE_)
        daal_memcpy_s(_min, rowSize, _cd.firstRow, rowSize);
#endif
#if (defined _MAX_ENABLE_)
        daal_memcpy_s(_max, rowSize, _cd.firstRow, rowSize);
#endif
   ...
   PRAGMA_IVDEP
   PRAGMA_VECTOR_ALWAYS
    for(size_t i = 0; i < _cd.nFeatures; i++) { _sums[i] = 0.0; _cd.variance[i] = 0.0; _cd.mean[i] = 0.0; }
#endif

/* Rows and features splitting by blocks */
#define _BLOCK_SIZE_ 256
    size_t numRowsInBlock = (_cd.nVectors > _BLOCK_SIZE_)?_BLOCK_SIZE_:_cd.nVectors;
    size_t numRowsBlocks   = _cd.nVectors / numRowsInBlock;
    size_t numRowsInLastBlock = numRowsInBlock + ( _cd.nVectors - numRowsBlocks * numRowsInBlock);

#define _FEATURE_BLOCK_SIZE_ 64
