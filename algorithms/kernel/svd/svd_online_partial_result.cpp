 ...
 */
void OnlinePartialResult::check(const daal::algorithms::Parameter *parameter, int method) const
{
    checkImpl(parameter, method, 0, 0);
}
 ...
 * \param[in] method Computation method
 */
void OnlinePartialResult::check(const daal::algorithms::Input* input, const daal::algorithms::Parameter *parameter, int method) const
{
    Input *svdInput   = static_cast<Input *>(const_cast<daal::algorithms::Input *>(input  ));
    checkImpl(parameter, method, svdInput->getNumberOfColumns(), svdInput->getNumberOfRows());
}

void OnlinePartialResult::checkImpl(const daal::algorithms::Parameter *parameter, int method, size_t nFeatures, size_t nVectors) const
{
    ...
    if(!checkNumericTable(firstNumTableInRCollection.get(), this->_errors.get(), rCollectionStr())) { return; }
    ...
    for(size_t i = 0 ; i < nodeSize ; i++)
    {
        ...
        if(!checkNumericTable(numTableInRCollection.get(), this->_errors.get(), rCollectionStr(), unexpectedLayouts, 0, nFeatures, nFeatures)) { return; }
        if(svdPar->leftSingularMatrix != notRequired)
        {
            ...
            if(!checkNumericTable(numTableInQCollection.get(), this->_errors.get(), qCollectionStr(), unexpectedLayouts, 0, nFeatures)) { return; }
