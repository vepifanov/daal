...
size_t DistributedStep2Input::getNumberOfColumns() const
{
    ...
    {
        this->_errors->add(Error::create(ErrorNullInputDataCollection, ArgumentName, SVDNodeCollectionStr()));
        return 0;
    }
    ...
    if(firstNodeSize == 0)
    {
        this->_errors->add(Error::create(ErrorIncorrectNumberOfElementsInInputCollection, ArgumentName, SVDNodeCollectionStr()));
        return 0;
    }
    // check 1st NT in 1st dataCollection;
    if(!(*firstNodeCollection)[0])
    {
        this->_errors->add(Error::create(ErrorNullNumericTable, ArgumentName, SVDNodeCollectionNTStr()));
        return 0;
    }
    ...
    {
        this->_errors->add(Error::create(ErrorIncorrectElementInNumericTableCollection, ArgumentName, SVDNodeCollectionStr()));
        return 0;
    }
    if(!checkNumericTable(firstNumTableInFirstNodeCollection.get(), this->_errors.get(), SVDNodeCollectionNTStr())) { return 0; }
...
void DistributedStep2Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    for(size_t i = 0 ; i < nNodes ; i++)
    {
        ...
        for(size_t j = 0 ; j < nodeSize ; j++)
        {
            ...
            if(!checkNumericTable(numTableInNodeCollection.get(), this->_errors.get(), SVDNodeCollectionNTStr(), unexpectedLayouts, 0, nFeatures, nFeatures)) { return; }
        }
    }
}
