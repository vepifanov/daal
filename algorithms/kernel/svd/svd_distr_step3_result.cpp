...
void DistributedPartialResultStep3::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(svdPar->leftSingularMatrix == requiredInPackedForm)
    {
        ...
        {
            if(!checkNumericTable(get(finalResultFromStep3)->get(leftSingularMatrix).get(), this->_errors.get(), leftSingularMatrixStr(),
...
void DistributedPartialResultStep3::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(svdPar->leftSingularMatrix == requiredInPackedForm)
    {
        ...
        {
            int unexpectedLayouts = (int)packed_mask;
            if(!checkNumericTable(get(finalResultFromStep3)->get(leftSingularMatrix).get(), this->_errors.get(), leftSingularMatrixStr(), unexpectedLayouts)) { return; }
        }
    }
}
