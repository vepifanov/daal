            ...
            const daal::algorithms::Parameter *par)
{
    ...
    SmartPtr<cpu> nonEmptyClassMapBuffer(nClasses * sizeof(size_t));
    size_t *nonEmptyClassMap = (size_t *)nonEmptyClassMapBuffer.get();
    getNonEmptyClassMap(nClasses, model, nonEmptyClassMap);
    ...
    daal::threader_for(nBlocks, nBlocks, [&](size_t iBlock)
    {
        ...
        getBlockOfRowsOfResults(nFeatures, startRow, nRows, nClasses, nonEmptyClassMap, mtX, mtR,
...
getNonEmptyClassMap(size_t &nClasses, const Model *model, size_t *nonEmptyClassMap)
{
    SmartPtr<cpu> nonEmptyClassBuffer(nClasses * sizeof(bool));
    ...
    for (size_t i = 1, imodel = 0; i < nClasses; i++)
    {
        ...
        {
            bool ijModelNotEmpty(model->getTwoClassClassifierModel(imodel));
                        ...
                        services::Error &error)
{
    ...
    get2ClassProbabilities(nFeatures, startRow, nRows, nClasses, nonEmptyClassMap, mtX, y,
    ...
    for (size_t k = 0; k < nRows; k++, rProbPtr += nClassesSq)
    {
        ...
        label[0] = (int)nonEmptyClassMap[0];
        for (int j = 1; j < nClasses; j++)
        {
            if (p[j] > maxProb)
            {
                maxProb = p[j];
                label[0] = (int)nonEmptyClassMap[j];
            }
        }
        mtR.release();
    }
