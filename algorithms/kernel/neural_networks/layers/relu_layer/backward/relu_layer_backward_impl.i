...
void ReLUKernel<algorithmFPType, method, cpu>::compute(Tensor *inputGradientTensor, Tensor *forwardDataTensor, Tensor *resultTensor)
{
    ...
    {
        dnnLayout_t inputLayout = (dnnLayout_t)forwardDataMklTensor->getLayout();
        dnnLayout_t inputGradLayout = (dnnLayout_t)inputGradientMklTensor->getLayout();
        ...
        resultMklTensor->setLayout(resultLayout);

        algorithmFPType* reluRes[dnnResourceNumber] = {0};

        reluRes[dnnResourceDiffDst] = inputGradientMklTensor->getArray();
        reluRes[dnnResourceDiffSrc] = resultMklTensor->getArray();
        reluRes[dnnResourceSrc] = forwardDataMklTensor->getArray();
    ...
    else
    {
        ...
        if (forwardDataMklTensor != 0)
        {
            forwardDataMklTensor->setPlainLayout();
        }
        if (resultMklTensor != 0)
        {
            resultMklTensor->setPlainLayout();
        }
