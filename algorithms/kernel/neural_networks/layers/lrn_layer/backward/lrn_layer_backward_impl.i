        ...
        const lrn::Parameter *parameter)
{
    ...
    if (sMinusBetaMklTensor != 0 && !sMinusBetaMklTensor->isPlainLayout())
    {
        ...
        {
            inputLayout = (dnnLayout_t)auxDataMklTensor->getLayout();
            lrnRes[dnnResourceSrc] = auxDataMklTensor->getArray();
        }
        else
        {
        ...
        {
            inputGradLayout = (dnnLayout_t)inputGradientMklTensor->getLayout();
            lrnRes[dnnResourceDiffDst] = inputGradientMklTensor->getArray();
        }
        else
        {
        ...
        sMinusBetaMklTensor->setLayout(workspaceLayout);
        lrnRes[dnnResourceWorkspace] = sMinusBetaMklTensor->getArray();
        ...
        {
            gradientMklTensor->setLayout(gradLayout);
            lrnRes[dnnResourceDiffSrc] = gradientMklTensor->getArray();
    ...
    else
    {
        if (auxDataMklTensor != 0)
        {
            auxDataMklTensor->setPlainLayout();
        }

        if (inputGradientMklTensor != 0)
        {
            inputGradientMklTensor->setPlainLayout();
        }

        if (sMinusBetaMklTensor != 0)
        {
            sMinusBetaMklTensor->setPlainLayout();
        }

        if (gradientMklTensor != 0)
        {
            gradientMklTensor->setPlainLayout();
        }
