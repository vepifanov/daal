...
DAAL_EXPORT void Result::allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, const int method)
{
    ...
    {
        set(layers::forward::value, services::SharedPtr<Tensor>(new MklTensor<algorithmFPType>(in->get(layers::forward::data)->getDimensions())));
    }
    ...
    {
        set(lrn::auxSmBeta, services::SharedPtr<Tensor>(new MklTensor<algorithmFPType>(in->get(layers::forward::data)->getDimensions())));
    }
