...
void Convolution2dKernel<algorithmFPType, method, cpu>::initialize(bool resultFlag, bool wDerFlag, bool bDerFlag)
{
    ...
    const convolution2d::Parameter *parameter, Tensor *wDerTensor, Tensor *bDerTensor, Tensor *resultTensor)
{
    ...
    {
        inGradMklTensor->setLayout((void*)inGradLayout);
        convRes[dnnResourceDiffDst] = inGradMklTensor->getArray();
    }
    else
    {
    ...
    if(convGrad)
    {
        ...
        if (resultMklTensor != 0)
        {
            ...
            convRes[dnnResourceDiffSrc] = resultMklTensor->getArray();
    ...
    if(convFilt)
    {
        ...
        if (xMklTensor != 0)
        {
            xMklTensor->setLayout(inputLayout);
            convRes[dnnResourceSrc] = xMklTensor->getArray();
        }
        else
        {
