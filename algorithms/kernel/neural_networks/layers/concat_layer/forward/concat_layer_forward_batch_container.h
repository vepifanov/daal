...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    size_t nInputs = input->get(layers::forward::inputLayerData)->size();

    Tensor *resultTensor  = result->get(layers::forward::value).get();

    SmartPtr<cpu> inputBlock( nInputs * sizeof(Tensor *));
    Tensor **inputTensors = (Tensor **)inputBlock.get();
    if (!inputTensors)  { this->_errors->add(services::ErrorMemoryAllocationFailed); return; }
