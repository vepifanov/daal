...
{
public:
    void compute(Tensor *inputTensors[], Tensor *resultTensor, size_t nInputs);

private:
    const size_t _nRowsInBlock = 5000;

    inline void processBlock(Tensor *inputTensor, size_t nProcessedRows, size_t nRowsInCurrentBlock, Tensor *resultTensor);
    inline void processBlockInit(Tensor *inputTensor, size_t nProcessedRows, size_t nRowsInCurrentBlock, Tensor *resultTensor);
};
