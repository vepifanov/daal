 ...
 */
SharedPtr<Tensor> Input::get(LayerDataId id) const
{
 ...
 */
void Input::set(LayerDataId id, const SharedPtr<Tensor> &value)
{
    LayerDataPtr layerData =
        layers::LayerData::cast<SerializationIface>(Argument::get(layers::backward::inputFromForward));
    (*layerData)[id] = value;
...
void Input::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    if (!checkTensor(auxDataTensor.get(), this->_errors.get(), dataStr())) { return; }
    const Collection<size_t> &inputDims = auxDataTensor->getDimensions();

    if (!checkTensor(auxGroundTruthTensor.get(), this->_errors.get(), groundTruthStr())) { return; }
...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    if (!checkTensor(get(layers::backward::gradient).get(), this->_errors.get(), gradientStr(), &gradDims)) { return; }
}
