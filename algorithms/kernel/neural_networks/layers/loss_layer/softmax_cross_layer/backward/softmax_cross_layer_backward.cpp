...
void Input::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    if (!data_management::checkTensor(auxProbabilitiesTensor.get(), this->_errors.get(), auxProbabilitiesStr())) { return; }
    ...
    if (!data_management::checkTensor(auxGroundTruthTensor.get(), this->_errors.get(), auxGroundTruthStr(), &groundTruthDims)) { return; }
}
...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    if (!data_management::checkTensor(get(layers::backward::gradient).get(), this->_errors.get(), gradientStr(), &gradDims)) { return; }
}
