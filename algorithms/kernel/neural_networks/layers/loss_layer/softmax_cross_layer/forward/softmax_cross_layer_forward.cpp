...
void Input::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    if (!data_management::checkTensor(groundTruthTensor.get(), this->_errors.get(), groundTruthStr())) { return; }
    ...
    if (!data_management::checkTensor(groundTruthTensor.get(), this->_errors.get(), groundTruthStr(), &groundTruthDims)) { return; }
}
