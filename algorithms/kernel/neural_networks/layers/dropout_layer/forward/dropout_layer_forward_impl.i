...
void DropoutKernel<algorithmFPType, method, cpu>::initialize(const dropout::Parameter *parameter)
{
...
template<typename algorithmFPType, Method method, CpuType cpu>
void DropoutKernel<algorithmFPType, method, cpu>::reset()
{
    baseRng.reset();
    rngs.reset();
}
...
template<typename algorithmFPType, Method method, CpuType cpu>
void DropoutKernel<algorithmFPType, method, cpu>::compute(
    Tensor *inputTensor,
    Tensor *resultTensor,
    Tensor *maskTensor,
    const dropout::Parameter *parameter)
{
    if (parameter->predictionStage && inputTensor == resultTensor) { return; }
    ...
    if (!rngBuffer.get()) { this->_errors->add(ErrorMemoryAllocationFailed); return; }
