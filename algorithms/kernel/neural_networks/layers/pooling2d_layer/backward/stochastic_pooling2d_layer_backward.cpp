/* file: stochastic_pooling2d_layer_backward.cpp */
/*******************************************************************************
...
{
__DAAL_REGISTER_SERIALIZATION_CLASS(Result, SERIALIZATION_NEURAL_NETWORKS_LAYERS_STOCHASTIC_POOLING2D_BACKWARD_RESULT_ID);
/**
 * Default constructor
 */
Input::Input() {}
...
void Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(!data_management::checkTensor(get(stochastic_pooling2d::auxSelectedIndices).get(), this->_errors.get(), auxSelectedIndicesStr(),
                                     &inputGradDims)) { return; }
