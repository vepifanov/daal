        ...
        inline void compute_max( int n, int offset, const float* parg, float* pmax, int* pmaxidx )
        {
            ...
            else
            {
                ...
                for(int i = 0; i < n; i++)
                {
                    ...
                    __m128i mmask = _mm_cmpgt_epi32( _mm_castps_si128(ma), _mm_castps_si128(mmax) );
        ...
        inline void compute_max( int n, int offset, const double* parg, double* pmax, int* pmaxidx )
        {
            ...
            else
            {
                ...
                for(int i = 0; i < n; i++)
                {
                    ...
                    __m128i mmask = _mm_cmpgt_epi64( _mm_castpd_si128(ma), _mm_castpd_si128(mmax) );
        ...
        Tensor *selectedPosTensor, const Parameter *parameter)
{
    ...
    if (dataMklTensor != NULL && selectedPosMklTensor != NULL)
    {
        ...
        inputLayout = (dnnLayout_t)dataMklTensor->getLayout();
        maxPoolRes[dnnResourceSrc] = dataMklTensor->getArray();
        ...
        if (valueMklTensor != NULL)
        {
            ...
            valueMklTensor->setLayout(resultLayout);
            maxPoolRes[dnnResourceDst] = valueMklTensor->getArray();
    ...
    else
    {
        ...
        {
            dataMklTensor->setPlainLayout();
        }
        if (valueMklTensor != NULL)
        {
            valueMklTensor->setPlainLayout();
        }
        if (selectedPosMklTensor != NULL)
        {
...
void PoolingKernel<algorithmFPType, method, cpu>::indicesLastZeroPaddingsCompute(
                ...
                int *selectedPos)
{
    const algorithmFPType minValue = -(data_feature_utils::internal::MaxVal<algorithmFPType, cpu>::get());
        ...
        for (DAAL_INT f = 0, fo = 0; fo < par.firstOutSize; f += par.firstStride, fo++)
        {
            ...
            for (DAAL_INT ffi = f; ffi < fUpper; fi++, ffi++)
            {
                ...
                for (; so < par.secondOutSize; so++, valueIndex++, s += par.secondStride)
                {
                     ...
                     * Input data index
                     */
                    const algorithmFPType *dataPtr = dataPtrBase + s;
                     ...
                     */
                    for (int si = 0; si < sUpper; si++, dataPtr++)
                    {
                        if (*dataPtr > value[valueIndex])
                        {
                            value[valueIndex] = *dataPtr;
                        }
                    }
