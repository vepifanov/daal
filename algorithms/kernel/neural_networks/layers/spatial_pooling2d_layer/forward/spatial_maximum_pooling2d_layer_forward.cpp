...
{
__DAAL_REGISTER_SERIALIZATION_CLASS(Result, SERIALIZATION_NEURAL_NETWORKS_LAYERS_SPATIAL_MAXIMUM_POOLING2D_FORWARD_RESULT_ID);
/** Default constructor */
Result::Result() {}
...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(!param->predictionStage)
    {
        ...
        if (!data_management::checkTensor(get(spatial_maximum_pooling2d::auxSelectedIndices).get(), this->_errors.get(), auxSelectedIndicesStr(),
                                          &valueDimensions)) { return; }

        const services::Collection<size_t> &dataDims = in->get(layers::forward::data)->getDimensions();
        data_management::NumericTablePtr auxInputDimensions = get(spatial_maximum_pooling2d::auxInputDimensions);
