/* file: spatial_stochastic_pooling2d_layer_backward.cpp */
/*******************************************************************************
...
#include "spatial_stochastic_pooling2d_layer_backward_types.h"
...
{
__DAAL_REGISTER_SERIALIZATION_CLASS(Result, SERIALIZATION_NEURAL_NETWORKS_LAYERS_SPATIAL_STOCHASTIC_POOLING2D_BACKWARD_RESULT_ID);
/**
 * Default constructor
 */
Input::Input() {}
...
void Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(!data_management::checkTensor(get(spatial_stochastic_pooling2d::auxSelectedIndices).get(), this->_errors.get(), auxSelectedIndicesStr(), &inputGradDims)) { return; }
    if(!data_management::checkNumericTable(get(auxInputDimensions).get(), this->_errors.get(), auxInputDimensionsStr(), data_management::packed_mask, 0, 4, 1)) { return; }
}
