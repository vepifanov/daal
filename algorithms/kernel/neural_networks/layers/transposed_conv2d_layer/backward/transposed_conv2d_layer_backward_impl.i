...
void TransposedConv2dKernel<algorithmFPType, method, cpu>::compute(Tensor *inGradTensor, Tensor *xTensor, Tensor *wTensor,
        const transposed_conv2d::Parameter *parameter, Tensor *wDerTensor, Tensor *bDerTensor, Tensor *resultTensor)
{
...

    {
        ...
        if(dconvKernel.getErrorCollection()->size() != 0) {this->_errors->add(dconvKernel.getErrorCollection()); return;}
    }
    ...
    if(parameter->propagateGradient) // compute gradient w.r.t. data
    {
...

        {
            ...
            convKernel.reset();
            if(convKernel.getErrorCollection()->size() != 0) {this->_errors->add(convKernel.getErrorCollection()); return;}
        }
    }
}
