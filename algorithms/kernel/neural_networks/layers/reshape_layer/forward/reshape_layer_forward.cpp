...
{
    layers::forward::Result::check(input, parameter, method);
    ...
    if(!data_management::checkTensor(get(layers::forward::value).get(), this->_errors.get(), valueStr(), &outDims)) { return; }

    if(!par->predictionStage)
    {
        data_management::NumericTablePtr dimsNT = get(auxInputDimensions);
        if (dimsNT.get() == 0) { this->_errors->add(services::ErrorNullNumericTable); return; }
        if (dimsNT->getNumberOfColumns() != outDims.size()) { this->_errors->add(services::ErrorIncorrectNumberOfColumnsInOutputNumericTable); return; }
