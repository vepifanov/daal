/* file: maximum_pooling1d_layer_forward.cpp */
/*******************************************************************************
...
#include "maximum_pooling1d_layer_forward_types.h"
...
{
__DAAL_REGISTER_SERIALIZATION_CLASS(Result, SERIALIZATION_NEURAL_NETWORKS_LAYERS_MAXIMUM_POOLING1D_FORWARD_RESULT_ID);
/** Default constructor */
Result::Result() {}
...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(!param->predictionStage)
    {
        ...
        if (!data_management::checkTensor(get(maximum_pooling1d::auxSelectedIndices).get(), this->_errors.get(), auxSelectedIndicesStr(), &valueDimensions)) { return; }
