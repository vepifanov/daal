...
public:
    ...
    void compute(const NumericTable* y, const NumericTable* z, size_t p,
...
protected:
    ...
    void computeTestStatistics(const NumericTable* betas, const algorithmFPType* v,
        algorithmFPType alpha, algorithmFPType accuracyThreshold, SingleBetaOutput& out);
    bool computeRmsVariance(const NumericTable* y, const NumericTable* z, size_t p, NumericTable* rms, NumericTable* variance);
    bool computeInverseXtX(const NumericTable* xtx, bool bModelNe, NumericTable* xtxInv);
};
