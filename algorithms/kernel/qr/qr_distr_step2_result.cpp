...
void DistributedPartialResult::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(!checkNumericTable(firstNumTableInFirstNodeCollection.get(), this->_errors.get(), QRNodeCollectionNTStr())) { return; }
    ...
    for(size_t i = 0 ; i < nNodes ; i++)
    {
        ...
        for(size_t j = 0 ; j < nodeSize ; j++)
        {
            ...
            if(!checkNumericTable(rNumTableInNodeCollection.get(), this->_errors.get(), QRNodeCollectionNTStr(), unexpectedLayouts, 0, nFeatures, nFeatures)) { return; }
        }
    }
    int unexpectedLayouts = (int)packed_mask;
    if(get(finalResultFromStep2Master))
    {
        if(!checkNumericTable(get(finalResultFromStep2Master)->get(matrixR).get(), this->_errors.get(), matrixRStr(),
