...
/** Default constructor */
DistributedStep3Input::DistributedStep3Input() : daal::algorithms::Input(2) {}
...
void DistributedStep3Input::getSizes(size_t& nFeatures, size_t& nVectors) const
{
    ...
    if(!checkNumericTable(numTableInQCollection.get(), this->_errors.get(), qCollectionStr())) { return; }
 ...
 */
void DistributedStep3Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    DataCollectionPtr qCollection = get(inputOfStep3FromStep1);
    DataCollectionPtr rCollection = get(inputOfStep3FromStep2);
    DAAL_CHECK_EX(qCollection, ErrorNullInputDataCollection, ArgumentName, inputOfStep3FromStep2Str());
    ...
    size_t nFeatures = 0;
    size_t nVectors = 0;
    this->getSizes(nFeatures, nVectors);
