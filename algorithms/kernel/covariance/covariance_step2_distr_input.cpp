...
size_t DistributedInput<step2Master>::getNumberOfFeatures() const
{
    ...
    {
        SharedPtr<PartialResult> onePartialResult =
...
void DistributedInput<step2Master>::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    {
        SharedPtr<PartialResult> partialResult =
        ...
        if(!checkNumericTable(nObservationsTable, this->_errors.get(), nObservationsStr(), csrLayout, 0, 1, 1)) { return; }
        ...
        if(!checkNumericTable(crossProductTable, this->_errors.get(), crossProductStr(), crossProductUnexpectedLayout, 0, nFeatures,
                                               nFeatures)) { return; }

        /* Check partial sums */
        NumericTable *sumTable = static_cast<NumericTable *>(partialResult->get(sum).get());
        if(!checkNumericTable(sumTable, this->_errors.get(), sumStr(), packedLayouts, 0, nFeatures, 1)) { return; }
    }
}
