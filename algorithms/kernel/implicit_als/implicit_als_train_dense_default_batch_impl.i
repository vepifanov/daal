    ...
    algorithmFPType *a, size_t *lda, algorithmFPType *b)
{
    ...
    char uplo = 'U';
    DAAL_INT iOne = 1;
    Blas<algorithmFPType, cpu>::xxsyr(&uplo, (DAAL_INT *)nCols, coeff, x, &iOne, a, (DAAL_INT *)lda);
    ...
    algorithmFPType *b, size_t *ldb)
{
    ...
    Lapack<algorithmFPType, cpu>::xxpotrf(&uplo, (DAAL_INT *)nCols, a, (DAAL_INT *)lda, &info);
    if ( info != 0 ) { this->_errors->add(services::ErrorALSInternal); return; }

    /* Solve L*L' * x = b */
    Lapack<algorithmFPType, cpu>::xxpotrs(&uplo, (DAAL_INT *)nCols, &iOne, a, (DAAL_INT *)lda, b, (DAAL_INT *)ldb, &info);
    if ( info != 0 ) { this->_errors->add(services::ErrorALSInternal); return; }
}
