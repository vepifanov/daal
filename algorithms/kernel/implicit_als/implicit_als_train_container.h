...
void DistributedContainer<step2Master, algorithmFPType, method, cpu>::compute()
{
    ...
    if (!partialCrossProducts) { this->_errors->add(services::ErrorMemoryAllocationFailed); return; }
    ...
    __DAAL_CALL_KERNEL(env, internal::ImplicitALSTrainDistrStep2Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType),
...
void DistributedContainer<step4Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::ImplicitALSTrainDistrStep4Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
