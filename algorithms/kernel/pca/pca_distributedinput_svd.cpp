...
DistributedInput<svdDense>::DistributedInput() : InputIface(1)
{
...
*/
void DistributedInput<svdDense>::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    for(size_t j = 0; j < nBlocks; j++)
    {
        ...
        if(!checkNumericTable(nObservationsSVD.get(), this->_errors.get(), nObservationsSVDStr(), csrLayout, 0, 1, 1)) { return; }

        NumericTablePtr sumSquaresSVD = getPartialResult(j)->get(pca::sumSquaresSVD);
        if(!checkNumericTable(sumSquaresSVD.get(), this->_errors.get(), sumSquaresSVDStr(), packedLayouts, 0, 0, 1)) { return; }
        ...
        if(!checkNumericTable(sumSVD.get(), this->_errors.get(), sumSVDStr(), packedLayouts, 0, nFeatures, 1)) { return; }
        ...
        for(size_t i = 0; i < auxiliaryData->size(); i++)
        {
            ...
            if(!checkNumericTable(table.get(), this->_errors.get(), auxiliaryDataStr(), csrLayout, 0, nFeatures, nFeatures)) { return; }
        }
    }
}
