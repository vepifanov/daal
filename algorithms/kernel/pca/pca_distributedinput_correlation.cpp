...
DistributedInput<correlationDense>::DistributedInput() : InputIface(1)
{
...
*/
void DistributedInput<correlationDense>::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    for(size_t j = 0; j < nBlocks; j++)
    {
        ...
        if(!checkNumericTable(nObservationsCorrelation.get(), this->_errors.get(), nObservationsCorrelationStr(), csrLayout, 0, 1, 1)) { return; }

        NumericTablePtr crossProductCorrelation = getPartialResult(j)->get(pca::crossProductCorrelation);
        if(!checkNumericTable(crossProductCorrelation.get(), this->_errors.get(), crossProductCorrelationStr(), packedLayouts)) { return; }

        size_t nFeatures = getPartialResult(0)->get(pca::crossProductCorrelation)->getNumberOfColumns();
        if(!checkNumericTable(crossProductCorrelation.get(), this->_errors.get(), crossProductCorrelationStr(), packedLayouts, 0, nFeatures, nFeatures)) { return; }
