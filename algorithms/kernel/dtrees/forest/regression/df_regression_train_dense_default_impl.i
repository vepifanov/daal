...
void subtractImpurity(algorithmFPType var, algorithmFPType mean,
...
public:
    ...
    {
        subtractImpurity<algorithmFPType, cpu>(total.var, total.mean, split.left.var, split.left.mean, split.nLeft,
            split.left.var, split.left.mean, n - split.nLeft);
    ...
    TResponse predict(const dtrees::internal::Tree& t, const algorithmFPType* x) const
    {
        const typename TreeType::NodeType::Base* pNode =
            dtrees::prediction::internal::findNode<algorithmFPType, TreeType, cpu>(t, x);
        DAAL_ASSERT(pNode);
    ...
    const ImpurityData& curImpurity, TSplitData& split) const
{
    ...
    intermSummFPType bestImpDecreasePart = split.impurityDecrease < 0 ? -1 :
    ...
    const algorithmFPType accuracy, const ImpurityData& curImpurity, TSplitData& split) const
{
    ...
    algorithmFPType vBest = split.impurityDecrease < 0 ?
        daal::data_feature_utils::internal::MaxVal<algorithmFPType>::get() : (curImpurity.var - split.impurityDecrease)*algorithmFPType(n);
    IndexType iBest = -1;
...
#ifdef DEBUG_CHECK_IMPURITY
        if(iStart == 0)
            checkImpurityInternal(aIdx + count, n - count, right);
#endif
...
public:
    ...
    Status finalizeOOBError(const NumericTable* resp, algorithmFPType* res, algorithmFPType* resPerObs) const
    {
        ...
        if(res)
            *res = _res/algorithmFPType(nPredicted);
        return Status();
    }
};
...
public:
    ...
    TrainBatchTask(HostAppIface* hostApp, const NumericTable *x, const NumericTable *y,
...
//////////////////////////////////////////////////////////////////////////////////////////
template <typename algorithmFPType, decision_forest::regression::training::Method method, CpuType cpu>
services::Status RegressionTrainBatchKernel<algorithmFPType, method, cpu>::compute(HostAppIface* pHostApp,
