...
public:
    ...
    size_t nSamples() const { return _nSamples; }
...
protected:
    ...
    TrainBatchTaskBase(const NumericTable *x, const NumericTable *y, const Parameter& par,
        ...
        _accuracy(daal::data_feature_utils::internal::EpsilonVal<algorithmFPType>::get()),
...
services::Status TrainBatchTaskBase<algorithmFPType, cpu>::init()
{
    ...
    return _dataHelper.init(_data, _resp, _aSampleToF.get());
}
...
public:
    ...
    void simpleSplit(size_t iTree, const algorithmFPType* featureVal, const IndexType* aIdx, typename super::ImpurityType& imp);
...
template <typename algorithmFPType, CpuType cpu>
void TrainBatchTaskBaseXBoost<algorithmFPType, cpu>::simpleSplit(size_t iTree, const algorithmFPType* featureVal,
    const IndexType* aIdx, typename super::ImpurityType& imp)
{
    const auto gh = grad(iTree);
