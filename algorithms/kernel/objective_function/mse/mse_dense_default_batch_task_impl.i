    ...
    ntArgument(argument)
{
    ...
    ntArgument->getBlockOfRows(0, argumentSize, readOnly, argumentBlock);
    ...
    algorithmFPType **hessian)
{
    ...
    if(hessianFlag)
    {
        ntHessian->getBlockOfRows(0, argumentSize, writeOnly, hessianBlock);
        *hessian = hessianBlock.getBlockPtr();
        if(!hessian) {error.setId(ErrorMemoryAllocationFailed); return;}
    }
    if(gradientFlag)
    {
        ntGradient->getBlockOfRows(0, argumentSize, writeOnly, gradientBlock);
