...
{
    DAAL_CHECK(par != 0, ErrorNullParameterNotSupported);
    if (!checkNumericTable(get(argument).get(), this->_errors.get(), argumentStr(), 0, 0, 1)) { return; }
}
...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    {
        if(!checkNumericTable(get(valueIdx).get(), this->_errors.get(), valueIdxStr(), 0, 0, 1, 1)) {return;}
    }
    if(algParameter->resultsToCompute & gradient)
    {
        if(!checkNumericTable(get(gradientIdx).get(), this->_errors.get(), gradientIdxStr(), 0, 0, 1, nRows)) {return;}
    }
    if(algParameter->resultsToCompute & hessian)
    {
        if(!checkNumericTable(get(hessianIdx).get(), this->_errors.get(), hessianIdxStr(), 0, 0, nRows, nRows)) {return;}
    }
}
