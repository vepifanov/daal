...
class MaxPool2DLayerDenseBatch {
    ...
    public static void main(String[] args) throws java.io.FileNotFoundException, java.io.IOException {
        ...
        Service.printTensor("Forward two-dimensional maximum pooling input (first 10 rows):", data, 10, 0);

        /* Create an algorithm to compute forward two-dimensional pooling results using default method */
        MaximumPooling2dForwardBatch maximumPooling2DLayerForward = new MaximumPooling2dForwardBatch(context, Double.class, MaximumPooling2dMethod.defaultDense, nDim);
        ...
        Service.printTensor("Forward two-dimensional maximum pooling result (first 5 rows):", forwardResult.get(ForwardResultId.value), 5, 0);
        Service.printTensor("Forward two-dimensional maximum pooling layer selected indices (first 5 rows):",
            forwardResult.get(MaximumPooling2dLayerDataId.auxSelectedIndices), 5, 0);

        /* Create an algorithm to compute backward two-dimensional pooling results using default method */
        MaximumPooling2dBackwardBatch maximumPooling2DLayerBackward = new MaximumPooling2dBackwardBatch(context, Double.class, MaximumPooling2dMethod.defaultDense, nDim);
        ...
        Service.printTensor("Backward two-dimensional maximum pooling result (first 10 rows):", backwardResult.get(BackwardResultId.gradient), 10, 0);
