                                                                   ...
                                                                   size_t *dataBlockPartition)
{
    ...
    size_t *nNotNull = new size_t[nBlocks];
...
void computePartialModelBlocksToNode(size_t nBlocks,
                                     ...
                                     KeyValueDataCollectionPtr *itemsOutBlocks)
{
    ...
    for (size_t i = 0; i < nBlocks; i++)
    {
        ...
        itemsPartition[i + 1] = itemsPartition[i] + transposedDataTable[i]->getNumberOfRows();
    }

    for (size_t i = 0; i < nBlocks; i++)
    {
        usersOutBlocks[i] = computeOutBlocks(nBlocks, dataTable[i],           itemsPartition);
        itemsOutBlocks[i] = computeOutBlocks(nBlocks, transposedDataTable[i], usersPartition);
    }
}
                     ...
                     NumericTablePtr ratings)
{
    ...
    double *ratingsData = ((HomogenNumericTable<double> *)ratings.get())->getArray();
