...
void printALSRatings(NumericTablePtr usersOffsetTable, NumericTablePtr itemsOffsetTable,
...
template<typename type1, typename type2>
void printTensors(TensorPtr dataTable1,
                  TensorPtr dataTable2,
                  const char *title1 = "", const char *title2 = "", const char *message = "", size_t nPrintedRows = 0, size_t interval = 15)
{
...
void printTensor3d(TensorPtr dataTable, const char *message = "",
                   size_t nFirstDim = 0, size_t nSecondDim = 0, size_t interval = 10)
{
    ...
    for (size_t i = 0; i < nFirstDim; i++)
    {
        ...
        printArray<DAAL_DATA_TYPE>(block.getPtr(), nThirdDim, nSecondDim, nThirdDim, "", interval);
...
TensorPtr getNextSubtensor(TensorPtr inputTensor, size_t startPos, size_t nElements)
{
    daal::services::Collection<size_t> dims = inputTensor->getDimensions();
    dims[0] = nElements;

    SubtensorDescriptor<DAAL_DATA_TYPE> subtensorBlock;
    inputTensor->getSubtensor(0, 0, startPos, nElements, readOnly, subtensorBlock);

    HomogenTensor<DAAL_DATA_TYPE> *subtensor = new HomogenTensor<DAAL_DATA_TYPE>(dims, Tensor::doAllocate);
    DAAL_DATA_TYPE *subtensorData = subtensor->getArray();
    DAAL_DATA_TYPE *inputData = subtensorBlock.getPtr();
    ...
    TensorPtr subtensorPtr(subtensor);
    return subtensorPtr;
}
