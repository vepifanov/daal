...
training::Distributed<step2Master> net;
training::Distributed<step1Local> netLocal[nNodes];
...
void initializeNetwork()
{
    ...
    net.initialize(dataDims, *topologyMaster);
    ...
    for(size_t node = 0; node < nNodes; node++)
    {
        ...
        netInit.initialize(dataDims, *(topology[node]));
...
void trainModel()
{
    ...
    for (size_t i = 0; i < nSamples - batchSizeLocal + 1; i += batchSizeLocal)
    {
        ...
        net.compute();
        checkPtr(net.getPartialResult().get());
        checkPtr(net.getPartialResult()->get(training::resultFromMaster).get());
        training::ModelPtr wbModel = net.getPartialResult()->get(training::resultFromMaster)->get(training::model);
        checkPtr(wbModel.get());
