...
int main()
{
    ...
    PackedSymmetricMatrix<NumericTableIface::lowerPackedSymmetricMatrix, double> dataTable(data, nDim);
    ...
    printArray<double>(block.getBlockPtr(), nDim, block.getNumberOfRows(), "Print 3 rows from packed symmetric matrix as double:");

    /* Read a feature(column) and write into it */
    readFeatureIdx = 2;
    dataTable.getBlockOfColumnValues(readFeatureIdx, firstReadRow, nDim, readWrite, block);
    printArray<double>(block.getBlockPtr(), 1, block.getNumberOfRows(), "Print the third feature of packed symmetric matrix:");
