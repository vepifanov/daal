...
static void runKmeans(const NumericTablePtr& inputData, size_t nClusters, const char* methodName, double oversamplingFactor = -1.0)
{
    ...
    kmeans::init::Batch<double, method> init(nClusters);
    ...
    /* Print the results */
    const double goalFunc = getSingleValue<double>(algorithm.getResult()->get(kmeans::goalFunction));
    const int nIterations = getSingleValue<int>(algorithm.getResult()->get(kmeans::nIterations));
    std::cout << "K-means algorithm results: goal function value = " << goalFunc*1e-6
