/* file: kmeans_init_csr_distr.cpp */
/*******************************************************************************
...
/*
!  Content:
!    C++ example of CSR K-Means clustering in the distributed processing mode
!******************************************************************************/

/**
* <a name="DAAL-EXAMPLE-CPP-KMEANS_INIT_CSR_DISTRIBUTED"></a>
* \example kmeans_init_csr_distr.cpp
*/
...
void loadData(NumericTablePtr data[nBlocks])
{
    for(size_t i = 0; i < nBlocks; i++)
        data[i] = services::SharedPtr<CSRNumericTable>(createSparseTable<double>(dataFileNames[i]));
}
...
NumericTablePtr initStep1(const NumericTablePtr data[nBlocks])
{
    for(size_t i = 0; i < nBlocks; i++)
    {
        ...
        kmeans::init::Distributed<step1Local, double, method> local(nClusters, nBlocks*nVectorsInBlock, i*nVectorsInBlock);
        local.input.set(kmeans::init::data, data[i]);
        local.compute();
        NumericTablePtr pNewCenters = local.getPartialResult()->get(kmeans::init::partialClusters);
        if(pNewCenters.get())
            return pNewCenters;
    }
...
void initStep23(const NumericTablePtr data[nBlocks], DataCollectionPtr localNodeData[nBlocks], const NumericTablePtr& step2Input,
    kmeans::init::Distributed<step3Master, double, method>& step3, bool bFirstIteration)
{
    for(size_t i = 0; i < nBlocks; i++)
    {
        kmeans::init::Distributed<step2Local, double, method> step2(nClusters, bFirstIteration);
...
NumericTablePtr initStep4(const NumericTablePtr data[nBlocks], DataCollectionPtr localNodeData[nBlocks],
    kmeans::init::Distributed<step3Master, double, method>& step3)
{
    ...
    for(size_t i = 0; i < nBlocks; ++i)
    {
        ...
        kmeans::init::Distributed<step4Local, double, method> step4(nClusters);
    ...
    services::SharedPtr<RowMergedNumericTable> pMerged(new RowMergedNumericTable());
...
NumericTablePtr initCentroids<kmeans::init::plusPlusCSR>(const NumericTablePtr data[nBlocks])
{
    ...
    services::SharedPtr<RowMergedNumericTable> pCentroids(new RowMergedNumericTable());
    ...
    kmeans::init::Distributed<step3Master, double, kmeans::init::plusPlusCSR> step3(nClusters);
...
NumericTablePtr initCentroids<kmeans::init::parallelPlusCSR>(const NumericTablePtr data[nBlocks])
{
    ...
    kmeans::init::Distributed<step5Master, double, kmeans::init::parallelPlusCSR> step5(nClusters);
    step5.input.add(kmeans::init::inputCentroids, pNewCentroids);
    /* Create an algorithm object for the step 3 */
    kmeans::init::Distributed<step3Master, double, kmeans::init::parallelPlusCSR> step3(nClusters);
    ...
    for(size_t i = 0; i < nBlocks; i++)
    {
        ...
        kmeans::init::Distributed<step2Local, double, kmeans::init::parallelPlusCSR> local(nClusters, false);
...
void calculateCentroids(const NumericTablePtr& initialCentroids, const NumericTablePtr data[nBlocks])
{
    ...
    for(size_t it = 0; it < nIterations; it++)
    {
        ...
        {
            /* Create an algorithm object for the K-Means algorithm */
            kmeans::Distributed<step1Local, double, kmeans::lloydCSR> localAlgorithm(nClusters, false);
    ...
    {
        /* Create an algorithm object for the K-Means algorithm */
        kmeans::Batch<double, kmeans::lloydCSR> localAlgorithm(nClusters, 0);
