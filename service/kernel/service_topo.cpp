...
static int __internal_daal_bindContext(unsigned int cpu, void *prevAffinity)
{
    ...
    *((DWORD_PTR *)prevAffinity)=SetThreadAffinityMask(GetCurrentThread(), affinity)
    if( (DWORD_PTR)(*prevAffinity) )
        ret = 0;
#endif
#endif
    return ret;
...
unsigned  int lcl_OSProcessorCount = 0;
    ...
    {   lcl_OSProcessorCount = 0;
        ...
        for( i = 0; i < grpCnt; i++ )
            lcl_OSProcessorCount += pSystem_rel_info->Group.GroupInfo[i].ActiveProcessorCount;
    }
#else
    {
        GetSystemInfo(&si);
...
static unsigned __internal_daal_slectOrdfromPkg(unsigned  package,unsigned  core, unsigned  logical)
{
    ...
    if( package != ENUM_ALL && package >= _internal_daal_GetSysProcessorPackageCount() )
        return 0;

    for( i=0; i< _internal_daal_GetOSLogicalProcessorCount(); i++ )
    {
        ////
        if( !(package == ENUM_ALL ||  glbl_obj.pApicAffOrdMapping[i].packageORD == package) )
            continue;

        if( !(core == ENUM_ALL || core == glbl_obj.pApicAffOrdMapping[i].coreORD) )
            continue;

        if( !(logical == ENUM_ALL || logical == glbl_obj.pApicAffOrdMapping[i].threadORD) )
            continue;
...
static int __internal_daal_findEachCacheIndex(DWORD maxCPUID, unsigned cache_subleaf)
{
    ...
    if( type > 0 )
    {
        ...
        {
            if(glbl_obj.cacheDetail[i].type == type)
                glbl_obj.cacheDetail[i].how_many_caches_share_level++;
        }
...
static void  __internal_daal_initStructuredLeafBuffers()
{
    ...
    memcpy(glbl_obj.cpuid_values[0].subleaf[0], &info, 4*sizeof(unsigned int));
    ...
    for( j=1; j <= maxCPUID; j++ )
    {
        ...
        memcpy(glbl_obj.cpuid_values[j].subleaf[0], &info, 4*sizeof(unsigned int));
        ...
        else if( j == 0x10 || j == 0xf )
        {
            ...
            while( kk < 32)
            {
                ...
                if( (qeidmsk  & (1 << kk) ) != 0 )
                    glbl_obj.cpuid_values[j].subleaf_max = kk;
                kk ++;
            }
        }
        else if( (j == 0x4 || j == 0xb) )
        {
            ...
            while (type && subleaf < MAX_CACHE_SUBLEAFS)
            {
                ...
                memcpy(glbl_obj.cpuid_values[j].subleaf[subleaf], &info, 4*sizeof(unsigned int));
                subleaf++;
                glbl_obj.cpuid_values[j].subleaf_max = subleaf;
            }
        }
...
static int __internal_daal_queryParseSubIDs(void)
{
    ...
    __internal_daal_allocateGenericAffinityMask(&glbl_obj.cpu_generic_processAffinity, lcl_OSProcessorCount);
    __internal_daal_allocateGenericAffinityMask(&glbl_obj.cpu_generic_systemAffinity, lcl_OSProcessorCount);
...
static int __internal_daal_analyzeEachCHierarchy(unsigned subleaf, unsigned  numMappings)
{
    ...
    pEachCIDs= (unsigned *)_alloca( numMappings * sizeof(unsigned ) );
    if(pEachCIDs == NULL) return -1;
    _INTERNAL_DAAL_MEMSET(pEachCIDs, 0xff, numMappings* sizeof(unsigned ));

    pThreadIDsperEachC = (unsigned *)_alloca( numMappings * sizeof(unsigned ));
    ...
    pDetectedEachCIDs= (unsigned *)_alloca( numMappings * sizeof(unsigned ) );
    if(pDetectedEachCIDs == NULL) return -1;
    _INTERNAL_DAAL_MEMSET(pDetectedEachCIDs, 0xff, numMappings* sizeof(unsigned ));

    pDetectThreadIDsperEachC = (unsigned *)_alloca( numMappings * maxCacheDetected * sizeof(unsigned ));
...
static void __internal_daal_buildSystemTopologyTables()
{
    ...
    if( __internal_daal_cpuTopologyParams() )
        return;
    if( __internal_daal_cacheTopologyParams() )
        return;
    ...
    if( glbl_obj.maxCacheSubleaf != -1 )
    {
        for( subleaf=0; subleaf <= glbl_obj.maxCacheSubleaf; subleaf++ )
        {
            if( glbl_obj.EachCacheMaskWidth[subleaf] != 0xffffffff )
            {
                ...
                if( __internal_daal_analyzeEachCHierarchy(subleaf, numMappings) < 0 )
                    glbl_obj.error |= _MSGTYP_TOPOLOGY_NOTANALYZED;
            }
        }
    }
...
unsigned _internal_daal_GetEnumerateAPICID(unsigned processor)
{
    ...
    if( processor >= glbl_obj.OSProcessorCount )
...
unsigned _internal_daal_GetEnumeratedCoreCount(unsigned  package_ordinal)
{
    ...
    if( glbl_obj.error || package_ordinal >= glbl_obj.EnumeratedPkgCount )
        return 0;
...
unsigned _internal_daal_GetEnumeratedThreadCount(unsigned  package_ordinal, unsigned  core_ordinal)
{
    ...
    if( core_ordinal >= glbl_obj.perPkg_detectedCoresCount.data[package_ordinal] )
        return 0;
...
unsigned _internal_daal_GetCoreCountPerEachCache(unsigned  subleaf, unsigned  cache_ordinal)
{
    ...
    if( glbl_obj.error || cache_ordinal >= glbl_obj.EnumeratedEachCacheCount[subleaf] )
        return 0;
    ...
    for( pkg=0; pkg < _internal_daal_GetSysProcessorPackageCount(); pkg++ )
    {
        for( i = 0; i < _internal_daal_GetSysEachCacheCount(0); i++ )
        {
            if( _internal_daal_GetCoreCountPerEachCache(0, i) > 0 )
            {
                ...
                {
                    if( glbl_obj.pApicAffOrdMapping[j].packageORD      == pkg  &&
                        glbl_obj.pApicAffOrdMapping[j].EachCacheORD[0] == i )
                    {
...
unsigned getL1CacheSize()
{
    ...
    if(glbl_obj.error || !glbl_obj.maxCacheSubleaf)
        return 0;
...
unsigned getNumElementsFitInMemory(size_t sizeofMemory, size_t sizeofAnElement, size_t defaultNumElements)
{
    ...
    if(n < 1)
        return sizeofMemory ? 1 : defaultNumElements;
